...
| BOO-13 | Boodschappen | Producteditor + "opslaan in catalogus?"-prompt bij nieuw item | Next | Should | M | 🔧 | BOO-5 | **Gebouwd (2026-06-27).** Migr. `0061` (`products.emoji`, **live** via MCP) — additief/nullable, is_member-RLS dekt 'm. Producteditor [`app/product/edit.js`](app/product/edit.js) op de gedeelde entity-editor ([`useEntityForm`](lib/useEntityForm.js) + [`Editor`](lib/ui.js)): naam, schap ([`CATEGORIES`](lib/groceryCatalog.js)), standaard-eenheid en **emoji** ([`EmojiPicker`](lib/ui.js)). [`useProducts.updateProduct`](lib/useProducts.js) schrijft naar de gedeelde `products`-rij → **huishouden-breed**. **Werkt voor álle catalogusproducten**: tik op een product in de Catalogus → editor (een bundel-/zoek-item wordt eerst aangemaakt = "opslaan", dán bewerken). **Prompt:** na een nieuw item toevoegen vraagt de Catalogus "even aankleden?" → editor. Bewerkte emoji toont in "Eerder gekozen" ([`resolveProductImage`](lib/productImage.js) leest `products.emoji`). **Rest:** (1) device-rooktest; (2) **foto-upload** als afbeelding (bucket + `photoPicker`) bewust als vervolg — nu is het visueel een emoji; (3) UX-copy fijnslijpen op toestel. |
