...
| PLT-11 | Platform | Account- & dataverwijdering in-app (AVG + store-vereiste) | Next | Must | M | ⏳ | — | **Nieuw (2026-07-07, backlog-doorlichting).** Play/App Store eisen in-app accountverwijdering; er is nu géén flow en review-bevinding Data-1 (2026-07-02) toont dat `auth.users`-delete stukloopt op FK's zonder delete-gedrag (alleen `0002` regelde dit). Sweep-migratie (bewuste keuze per tabel: SET NULL vs CASCADE) + DEFINER-RPC `delete_account` + gevarenzone in instellingen + storage-opruiming + RLS-scenario's. Launch-blokkerend nu de iOS-submit-config compleet is (IOS-1). Uitwerking: [plan 29](docs/plans/29-junior-uitwerking-backlog.md) S1. **Data-4 beslist (2026-07-09): anonimiseren** — voltooiingen naar "onbekend lid" (SET NULL), géén cascade; eerlijkheid/leaderboard blijven kloppen. Bouwen kan. |
