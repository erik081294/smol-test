...
| PERF-2 | Platform/UX | Waargenomen snelheid: instant tab-wissel (geen laad-flits) | Next | Should | M | ✅ | INF-8 | **Gebouwd** (in-memory SWR-cache `lib/dataCache.js` + `freezeOnBlur`). **Device-geverifieerd 2026-06-25 (moto):** elke tab-wissel direct, geen laad-flits. Web-soepelheid blijft een losse follow-up. |
| PERF-3 | Platform/perf | Bundle: phosphor per-icoon importeren (i.p.v. barrel) | Next | Should | S | ✅ | — | **Gebouwd** ([`lib/icons.js`](lib/icons.js#L14-L81): per-icoon subpath-imports i.p.v. de barrel, `MAP` identiek). **Device-geverifieerd 2026-06-25 (moto):** alle iconen renderen correct over alle schermen (geen ontbrekende/lege iconen na de refactor). [plan 16](docs/plans/16-performance-audit.md). |
...
| TKN-2 | Taken/UX | Jaarweergave — activiteit-heatmap | Next | Could | M | ✅ | TKN-1 | **Gebouwd** (heatmap uit `task_completions`, geen migratie). **Device-geverifieerd 2026-06-25 (moto):** onder Meer→Inzichten rendert het dag×maand-raster met "Minder/Meer"-legenda, persoon-filter (Iedereen/Erik/Erik2), categorie-chips en streak-stats. |
...
| UX-12 | Platform/UX | Back vanuit een via-"Meer"-geopende tab gaat naar Home i.p.v. Meer | Next | Should | S | ✅ | UX-2 | **Gebouwd** (`backBehavior="history"`). **Device-geverifieerd 2026-06-25 (moto):** hardware-back vanuit Meer keert terug naar de vorige tab (Boodschappen), niet Thuis-eerst. Web nog niet apart getest. |
...
| UX-23 | Platform/UX | Veeg-acties op de Vandaag-focus-taken | Next | Could | S | ✅ | UX-17 | **Gebouwd** ([`vandaag.js`](app/(tabs)/vandaag.js#L270) wrapt de focus-taken in `SwipeRow`). **Device-geverifieerd 2026-06-25 door Erik** (de veeg-gebaren). Kruisref UX-43. |
| UX-24 | Platform/UX | Laad-skeleton op het Thuis-dashboard | Next | Could | S | ✅ | UX-15 | **Gebouwd** ([`vandaag.js`](app/(tabs)/vandaag.js#L260): koud laden toont `ListSkeleton` i.p.v. blanco). **Device-geverifieerd 2026-06-25 (moto):** koud laden hangt niet blanco; widget-grid komt netjes op. |
...
| UX-43 | Platform/UX | Swipe-conventie uniformeren + verwijderen ontdekbaar | Next | Should | M | ✅ | UX-23 | **Gebouwd** ([`ui.js`](lib/ui.js#L391): één `SwipeRow`-conventie — `left`=verwijderen (fysiek rechts), `right`=uitstellen (fysiek links)). **Device-geverifieerd 2026-06-25 door Erik** (de veeg-gebaren). Conventie in `DESIGN.md` borgen blijft een losse doc-follow-up. [plan 18](docs/plans/18-ux-verbeterplan.md) B1/B2. |
