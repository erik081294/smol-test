...
| BOO-9 | Boodschappen | Barcode scannen → catalogus | 2 | Should | M | ⏳ | BOO-5, VOO-1 | **Volgende ronde (gekozen 2026-06-21).** Scan een streepjescode (`expo-camera` barcode-scanner) → match op de bestaande catalogus (`products` / globale Open Food Facts via `search_catalog`, koppelt op `barcode`/`code`) → één tik: op de boodschappenlijst (`useGroceries.add` met `catalog_product_id`) óf in de voorraad (`usePantry`, VOO-2). Onbekende code → "nieuw product" inline (hergebruikt de bestaande inline-product-flow). Hergebruikt álles wat er al is; meest voelbare nieuwe feature. Mogelijk migratie: `barcode`-kolom + index op `products`. |
| BOO-10 | Boodschappen | Bonnen bewerkbaar maken | 2 | Could | M | ⏳ | BOO-2 | Symmetrie met bewerkbare uitgaven (UX-3/`update_expense`): een opgeslagen bon openen toont nu read-only. `update_purchase`-RPC (regels atomair vervangen, naar analogie van `create_purchase`/`update_expense`) + form-hergebruik in `app/purchase/[id].js` (edit-modus = zelfde editor, voorgevuld). |
...
| MLT-3 | Maaltijden | Recept-foto | 2 | Could | S | ⏳ | MLT-2 | Kolom `recipes.photo_path` bestaat al (migratie `0016`), maar de recept-editor toont nog géén fotoveld. UI toevoegen via het foto-patroon uit `app/plant/[id].js` — doe dit samen met de **PhotoPicker**-extractie (STR-4): dan twee gebruikers, dus de gedeelde component betaalt zich terug. |
...
| PLT-1 | Platform | Notificaties & herinneringen | 2 | Should | M | 🔧 | — | **Plan 05 — trap 1 af; migratie `0018` (`push_tokens`) live (2026-06-20)**. Trap 1 lokaal (`expo-notifications`, `useNotifications`, prefs-scherm) werkt. **Trap 2 remote nu productie-klaar (2026-06-21):** `notify` opgesplitst in pure kern (`core.js`, event-router + handler-registry, 7 units in `tests/notify.test.js`) + impure schil (`index.ts`) met gedeeld-geheim-check (`x-notify-secret`, fail-closed + constant-time), idempotentie + audit (`push_deliveries`, migratie `0023`), batchen (≤100) en automatische opruiming van dode tokens (`DeviceNotRegistered`). `verify_jwt=false` in `config.toml`. **Rest = flip-on** (vereist Supabase-toegang): migraties pushen, secret zetten, `functions deploy notify`, Database Webhook op `tasks`, 2-account-toesteltest — exact in `docs/notify-setup.md`. |
| PLT-6 | Platform | Activiteiten-/wijzigingenfeed | 2 | Could | M | ⏳ | — | **Volgende ronde (gekozen 2026-06-21).** "Tim vinkte 'stofzuigen' af", "Erik voegde melk toe". Nu goedkoop: de voltooiingen-log `task_completions` (0012) + realtime bestaan al, en de nieuwe `push_deliveries`-audit (0023) kan dit voeden. Aanpak: óf afleiden uit bestaande logs/`created_at`/realtime-events, óf een lichte `activity`-tabel (insert-triggers per module). Gescopet via `can_view`/`is_member`. Pure formatter `lib/activity.js` (event → NL-regel) + een feed-scherm/Thuis-kaart. |
...
| UX-4 | Platform/UX | Dark mode afmaken | 2 | Could | M | ⏳ | UX-3 | Scaffold staat er al: `beeldstijl`-scherm + `lib/themePrefs.js` (`mode: systeem/licht/donker`, persistent) en **themebare** illustraties (`lib/illustrations.js`, palet-tokens). **Rest:** een theme-context die `lib/theme.js` `colors.*` runtime omschakelt (donker palet toevoegen), `userInterfaceStyle` loszetten, en de banner "binnenkort" weghalen. Eén centrale token-laag → schermen erven het automatisch (geen rauwe kleuren in `app/`). |
| INF-1 | Platform | Live-Supabase-verificatie + RLS-tests | 1 | Must | S | 🔧 | — | Migraties 0004–**0022** live (DB op `0022`); `0016`–`0021` toegepast 2026-06-20 en `0022` (`update_expense`) op 2026-06-21 via de Supabase-connector — keuken-loop, kosten-autodelen, push-tokens, inzichten, recurring-`pg_cron`, FK-indexen, bewerkbare uitgaven. **Nieuw `0023` (`push_deliveries`, PLT-1 trap 2) staat in de repo en moet nog gepusht** (`supabase db push`, idempotent) — zie `docs/HANDOVER.md`. Units **196 tests / 0 fail** (18 RLS-integratietests lokaal geskipt zonder secrets; groen tegen live DB met secrets). Resteert: `0023` pushen + RLS-tests met secrets + de handmatige 2-account-rooktest (`VERIFICATIE.md` Stap 3). |
...
> + **STR-4** PhotoPicker-extractie, **UX-4** dark mode afmaken. Bewust níét nu: AI-assistent
> over eigen data (AI-1) — geparkeerd voor een latere ronde.
