...
| INF-1 | Platform | Live-Supabase-verificatie + RLS-tests | Now | Must | S | 🔧 | — | Migraties `0001`–`0036` live; kern-RLS 13/13 (`docs/rls-connector-check.sql`); suite 587 (569 pass/18 skip). JS-RLS mét secrets live: 15/18 (3 vielen om op auth-rate-limit → **INF-12**). SEC-ronde breidt de suite uit (SEC-1/SEC-4) + refactort naar `create_household`. **Rest:** 18/18 via INF-12 + 2-account-rooktest (`VERIFICATIE.md`). |
...
| STR-4 | Platform/UX | Ontbrekende gedeelde componenten | — | Should | S | ✅ | — | **Af.** Gedeelde componenten in `lib/ui.js` (`AvatarSelect`/`EmojiPicker`/`ListSkeleton`/`Editor`/`SwipeRow`/…); alle foto-schermen op de gedeelde `offerImagePicker`. (→ archief) |
...
| INF-12 | Platform | RLS-integratiesuite: auth-rate-limit bij volledige run | Now | Could | S | ⏳ | INF-1 | **Bevinding (2026-06-25):** volledige `tests/rls.integration.test.js` raakt de Supabase auth-rate-limit (~45-60 logins/run) → laatste ~6 flakey. **Fix:** batch per groep, of backoff tussen `makeUser`-calls. Geen migratie. |
...
| UXR-1 | UX-review | Ontleding: Vandaag / overzicht | — | Should | S | ✅ | — | **Teardown ✅** (`vandaag.js` strak) → UX-23/UX-24. [plan 14](docs/plans/14-ux-module-teardown.md). (→ archief) |
...
| UXR-3 | UX-review | Ontleding: Boodschappen | — | Should | S | ✅ | — | **Teardown ✅** (`boodschappen.js` compleet; `blurOnSubmit` meegenomen). [plan 14](docs/plans/14-ux-module-teardown.md). (→ archief) |
