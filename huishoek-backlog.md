...
| AI-18 | Assistent | Gen-UI-vocabulaire (compositie i.p.v. opnieuw beginnen) + HITL-vervolgbeurt | Now | Should | M | 🔧 | AI-16 | **Gebouwd (2026-07-06, Erik-vraag "modulair? en AI-vervolg na akkoord?") → [plan 26](docs/plans/26-gen-ui-componenten.md) ronde 2.** (1) **Gedeeld vocabulaire:** [`_shared/tools/render.js`](supabase/functions/_shared/tools/render.js) — `chartNode`/`scheduleNode`/`choiceNode`-constructors mét automatische text-fallback; kosten/maaltijden componeren eruit; **roundtrip-contracttest** borgt dat elke constructor-output ongeschonden door de client-poortwachter komt (mutatie 100%). Nieuw component per module = paar regels compositie. (2) **Bevestigen is een beurt:** na een geslaagd akkoord (bundel = één beurt) volgt automatisch een vervolg-beurt — server bouwt de tekst deterministisch uit de opgeslagen done-rijen (`actionFollowUpMessage`, gesanitized); AI bevestigt kort + stelt vervolgstap voor (vervolgactie = wéér HITL); synthetische beurt onzichtbaar als bubble, wél LLM-context, stil bij fouten (guidelines §9/§10). **Edge v18 live (2026-07-06, MCP; byte-geverifieerd).** **Rest:** device-verificatie (confirm→reactie, bundel→één beurt); golden-cases voor de vervolg-beurt (trace-review). |
| AI-19 | Assistent | AI overal: tool-packs voor planten/huisdieren/voertuigen/tijdlijn/delen | Now | Should | L | ◐ | AI-17, AI-18 | **Fase A + B GEBOUWD (2026-07-06) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md).** 10 tools (5 read + 5 HITL-write) — de assistent dekt en bedient alle datamodules. Writes: additief, géén financiële boekingen, conflictcheck bij reserveren, tz-correct via client-offset; undo-whitelist +5; 20 golden-cases. Toolset 16→21 (≤25-metatest; tool-search bewust uitgesteld). **Fase C AF (2026-07-08, [plan 29](docs/plans/29-junior-uitwerking-backlog.md) A1):** pack-ratchets 76–82% → **92,8–97,8%** (delen 94,9 · voertuigen 97,0 · huisdieren 95,3 · planten 97,8 · tijdlijn 92,8; baselines herijkt, descriptions byte-identiek — geen eval-gate-impact). **Rest:** edge-deploy (nieuwe bundel-bestanden!), eval-gate-run (ORQ-key), device-verificatie. Fase A was: 5 read-tools op de bestaande pure logica (plantCare/petCare/vehicleCosts/timeline/reservations), gen-UI via het render.js-vocabulaire. Fase B: 5 additieve HITL-writes (géén financiële boekingen; `hasConflict` verplicht bij reserveren; visibility default household). Let op: kruist het tool-budget (~21) → expliciet tool-search-beslispunt (guidelines §1). Per tool golden-cases + ratchet-groep. |
| AI-20 | Assistent | Eval-volwassenheid: multi-turn-cases + NL-toon-judge + Maestro-AI-flow | Now | Should | M | 🔧 | AI-3, AI-18 | **GEBOUWD (2026-07-07, stream) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md) golf 1a / plan 28 sessie 1.** (1) **Multi-turn**: golden-schema draagt optioneel `turns` (rollen-whitelist, laatste = user; `question` XOR `turns`); runner bouwt `[system, …history, user]`; 4 cases — 2× vervolg-beurt-na-confirm (nota via de **productie**-`actionFollowUpMessage`, `expect.tools=[]`) + 2× choice-reply → `maaltijden_plannen`; `--dry-run` valideert alle 69 cases zonder API-call. (2) **NL-toon-judge** [`scripts/assistant-judge.mjs`](scripts/assistant-judge.mjs): NL-rubric + 3 few-shot-ankers → `toneScore`, **opt-in `--tone`** zolang er geen gekalibreerde baseline is; baseline onaangeroerd. (3) [`.maestro/06-assistent.yaml`](.maestro/06-assistent.yaml): chat→kaart→Doen→vervolg-beurt, self-cleanend (E2E-prefix); testID's `t-assistant-confirm`/`t-assistant-reject` toegevoegd. **Rest (live/Erik):** eval-run + `--update-baseline` (ORQ-key; baseline zegt nog 43 cases), toon-kalibratie via `--tone`, rooktest-flow 06 op het toestel. |
| AI-21 | Assistent | Tool-search/deferred loading ontwerpen (vóór DOC-1/AI-9) | Next | Should | M | ⏳ | AI-19 | **Nieuw (2026-07-09, besluit D10):** niet stil naar de 25-tools-drempel groeien — het tool-search-/deferred-loading-ontwerp klaarzetten vóórdat de DOC-1- en AI-9-tools landen (nu 21 tools; `assistantCoverage`-metatest bewaakt de drempel). Raakt de agent-loop + guidelines §1 → **eval-gate** bij elke wijziging in wat het model ziet. |
| AI-9 | Assistent | Geheugen v1 (pgvector hybrid + async extractie + beheer-UI) | Later | Could | L | ⏳ | AI-4 | Ronde H: migratie `assistant_memories` (**nummer-toewijzing: zie [plan 27](docs/plans/27-ontwikkelprogramma-juli.md) — TML-4/6 gaan voor; verifieer live via MCP `list_migrations`**; vector + 'dutch' tsvector, user/household-scope-RLS) + hybrid-RRF-RPC; `remember_fact` (HITL) + async extractor (aparte deployment, dedupe); beheer-scherm; memory-gebruik zichtbaar in chat. |
...
| PLT-8 | Platform | Wachtwoordloze login: e-mail-OTP (native-first) | Now | Should | M | 🔧 | PLT-7 | **GEBOUWD (2026-07-06, stream).** OTP-code als primair pad in welcome (wachtwoord blijft), `lib/otp.js` (mutatie 100%), naam-stap `/naam` na eerste login, join→OTP-voorselectie. **Rest:** Supabase-dashboard (Erik: e-mail-OTP aan + `{{ .Token }}` in de Magic-Link-template) + device-e2e. Ontwerp: `signInWithOtp` + 6-cijferige code (géén magic-link-URL: native `detectSessionInUrl` uit, deep-links dekken alleen /join — gevalideerd). Raakt lib/auth.js, welcome.js, join-goLogin; display-name na eerste login. **Erik-actie:** Supabase-dashboard (OTP aan, template, redirect-allowlist). |
| PLT-3 | Platform | Globaal zoeken over modules heen | Now | Should | M | 🔧 | — | **GEBOUWD + migratie 0077 LIVE (2026-07-06, stream; advisor schoon).** `global_search`-RPC (INVOKER, LIKE-escaping, 8 bronnen) + `lib/searchRank.js` (92,6%) + `app/zoeken.js` (debounced, gegroepeerd) + ingangen op Thuis/Meer. **Rest:** device-verificatie. Ontwerp: RPC `global_search(q)` (SECURITY INVOKER — RLS scopet) over 8 tabellen + pure ranker `lib/searchRank.js` + zoekscherm via Meer/Thuis. Migratienummer: zie plan 27-toewijzing. |
| DOC-1 | Documenten | Documenten- & garantiekluis (nieuwe module) | Later | Should | L | ⏳ | — | **§7 → rij (2026-07-06, gekozen) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md) slot-golf.** `documents`-tabel + bucket (plantfoto-patroon), garantie-herinnering via de reminder-laag, editor via useEntityForm; direct AI-inclusief (`documenten_zoeken`-read-tool). |
...
| BOO-9 | Boodschappen | Barcode scannen → catalogus | Next | Should | M | ◐ | BOO-5, VOO-1 | **Datalaag af; scanner-UI device-gated** (`lib/barcode.js`/`barcodeLookup.js`/`openFoodFacts.js`, RPC `insert_catalog_product`, migr. `0027`/`0031`). **Bevinding 2026-06-26:** de scan-trigger leeft in de **bon-flow** ([`app/purchase/[id].js`](app/purchase/[id].js#L117) `onScanPress` → `offerImagePicker` → barcode uit de foto), niet als losse live `expo-camera`-scanner; die bon-editor heeft bovendien zelf nog geen UI-entry-point (zie BOO-10). **Rest:** of een eigen live-camera-scanknop in boodschappen/catalogus, óf eerst BOO-10's entry-point; dán end-to-end scannen op toestel. **Correctie (2026-07-07, [plan 29](docs/plans/29-junior-uitwerking-backlog.md) F5, code-geverifieerd):** de bon-flow-`onScanPress` is de bon-**OCR** (BOO-7/`scan-receipt`), geen barcode-pad; `lookupBarcode` heeft géén enkele aanroeper en `expo-camera` is geen dependency — de complete barcode-laag wacht dus op een scanner-UI (uitwerking in plan 29 F5). |
| BOO-14 | Boodschappen | Lijst krijgt meer schermruimte (verken-item, toestelfeedback 2026-06-28) | Next | Should | M | ◐ | — | **Stap 1 (2026-06-28):** de vaste "Catalogus openen"-knop + bonnen-link samengevoegd tot één compacte rij. **Stap 2 (2026-06-29):** "Misschien weer nodig" is nu **inklapbaar** (optie 3) — standaard ingeklapt zodat de lijst de schermruimte krijgt; de kop toont het aantal en klapt op één tik uit (de horizontale kaarten-rail blijft edge-to-edge). [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js). **Open (kies/combineer):** (1) kop laten inklappen bij scrollen (compacte titel, subtitle weg/samengevoegd); (2) catalogus/bonnen naar header-actie — let op: sluit op **UX-42** (kop-rechts = alleen uitleg/gelabelde acties, geen losse navigatie). **Rooktest 2026-06-30:** stap 1 (compacte "Catalogus \| Bonnen"-rij) device-bevestigd; stap 2 (inklap) is **code-bevestigd maar niet op toestel reproduceerbaar** — de sectie is data-gated op aankoopfrequentie (`dueScore ≥ 1`), die het testhuishouden niet heeft (bewust geen aankoophistorie op de live DB gefabriceerd). UX-review (`docs/ux-review-rooktest-2026-06-30.md`) → zie **UXR-10**. **Besluit (2026-07-09, D5):** opties (1)/(2) vervallen — de ruimtevraag gaat op in **BOO-18** (boodschappen-modus). **Rest:** alleen nog de stap-2-device-check (data-gated → INF-14). |
| BOO-18 | Boodschappen | Boodschappen-modus: full-screen lijst + deelbare gast-link | Next | Should | L | ⏳ | PLT-7 | **Nieuw (2026-07-09, besluit D5/D11).** Twee varianten op één lijst-ervaring: (1) **in-app full-screen afvinkmodus** ("boodschappen doen"); (2) **deelbare web-URL buiten het huishouden** — een gast kan de lijst zien, afvinken, items toevoegen/verwijderen en in de catalogus zoeken, maar géén vaste huishoud-producten aanmaken; een ingelogd lid wordt herkend en doorgeleid naar de volledige app. Leunt op het invite-token-patroon (migr. `0053`: DEFINER-RPC's + anon-`peek`) en de web-host; dekt het boodschappen-deel van PLT-9 (§7). **Security-review verplicht** (anon-schrijfpad op de lijst — rate-limits + token-scope à la `peek_invite`). Build-ready uitwerking nog te schrijven (plan-29-opvolger). |
...
| TML-4 | Tijdlijn | Tekstreacties/comments (alléén op berichten) | Now | Should | M | 🔧 | TML-1 | **GEBOUWD + migratie `0075` LIVE (2026-07-06, stream; advisor schoon).** `timeline_comments` erft post-zichtbaarheid (can_view op parent, óók bij INSERT — lekpreventie à la 0067), thread + invoer op het detail, `orderComments` (timeline-ratchet 93,8→95,2). RLS-scenario's in de suite. **Rest:** device-verificatie + realtime-check. |
| TML-6 | Tijdlijn | Filterinstellingen (per module + event-type, twee lagen) | Now | Should | M | 🔧 | TML-5 | **GEBOUWD + migratie `0076` LIVE (2026-07-06, stream; advisor schoon).** `lib/timelineFilter.js` (DEFAULT-ON, huishouden wint; 93,8%) + prefs-tabellen (RLS à la 0004, assen member/subgroup alvast in de CHECK voor TML-7/8) + filterscherm via de tijdlijn-header. **Rest:** device-verificatie. |
| TML-7 | Tijdlijn | Filter per persoon/lid | Now | Could | S | 🔧 | TML-6 | **GEBOUWD (2026-07-08, [plan 29](docs/plans/29-junior-uitwerking-backlog.md) F2; geen migratie — de 0076-CHECK dekte `member` al).** `visibleOnTimeline` kreeg de member-as (waarde = profiel-id); `actorId` reist mee door `useActivity`→`formatActivity`; het filter raakt berichten (auteur) én systeem-events (actor) op het tijdlijn-scherm; "Per lid"-sectie met avatars in het filterscherm (zelfde twee-lagen-gedrag, huishouden wint). Units: member-as-scenario's + actorId-doorreis; timelineFilter-ratchet gehouden. **Rest:** device-verificatie (lid uitzetten → posts+events weg; locked-gedrag voor niet-owner). Plan [`19`](docs/plans/19-tijdlijn-prikbord.md). |
| TML-8 | Tijdlijn | Filter per zichtbaarheid/subgroep | Later | Could | M | ⏳ | TML-6 | **Geparkeerd (besluit 2026-07-09, D9):** wachten tot subgroepen echt gebruikt worden; TML-7 dekt het meeste. Vorm bij bouwen: alléén "subgroep verbergen" (geen isolatie-modus). `axis='subgroup'` — **weergave**-filter bovenop de RLS. Plan [`19`](docs/plans/19-tijdlijn-prikbord.md). |
...
| PLT-11 | Platform | Account- & dataverwijdering in-app (AVG + store-vereiste) | Now | Must | M | 🔧 | — | **GEBOUWD + migratie `0078` LIVE (2026-07-09, plan 29 S1; advisor schoon).** Data-1-gat dicht: 22 NO ACTION-FK's naar `profiles` → SET NULL (anonimiseren, "onbekend lid"), `timeline_reactions` → CASCADE; de 0070-rekey-guard versoepeld (creator→NULL toegestaan, waarde→andere waarde blijft geblokkeerd — 0070-regressietest groen). DEFINER-RPC's `delete_account()` (owner-blokkade P0001 + solo-huishoud-cascade + `auth.users`-delete) en `account_deletion_preview()` (voedt pure [`lib/accountDeletion.js`](lib/accountDeletion.js) `classifyHouseholds`, mutatie 97,2%); gevarenzone in [instellingen](app/(tabs)/instellingen.js) → [`app/account-verwijderen.js`](app/account-verwijderen.js) (impact-preview + getypte "VERWIJDER"-bevestiging). **Live-RLS-geverifieerd**: preview-tellingen, owner-blokkade, anon-weigering én de anonimisering (lid verwijdert account → gedeelde taak blijft, `created_by` NULL, huisgenoot ziet 'm nog). **Bewust geaccepteerd (in de migratie gedocumenteerd):** `paid_by`→NULL laat betalingen van een vertrokken lid ongeattribueerd (AVG-defensief); posts/comments blijven met anonieme auteur (vrije tekst bewaard). **Rest:** device-rooktest van de flow; solo-owner die zijn huishouden-data ook echt weg wil (nu: blokkade → eerst beheer overdragen of huishouden verwijderen). |
| PLT-4 | Platform | Export als deelbare tekst/CSV | Now | Could | S | 🔧 | — | **GEBOUWD (2026-07-09, plan 29 F9).** Pure [`lib/export.js`](lib/export.js): `groceriesAsText` (deelbare boodschappenlijst, alleen open items) + `balancesAsCsv` (NL-Excel: puntkomma + komma-decimaal + CSV-escaping); unit-getest, ratchet-groep 90,0%. Via `Share.share` als gelabelde drawer-actie op Boodschappen ("Lijst delen") en Kosten ("Saldo exporteren") — UX-42-contract. **Rest:** device-rooktest van de sharesheet. |
| INF-14 | Platform | Seed-script demo-huishouden voor device-verificatie | Next | Should | S | ⏳ | — | **Nieuw (2026-07-07, backlog-doorlichting).** Terugkerende 🔧-blokkade is ontbrekende testdata, geen ontbrekend werk: huisdier-parity (PLA-10/HUI-1), BOO-14 stap 2 (`dueScore`-gated), PERF-5 (lege voorraad). Eén idempotent seed-/cleanup-script (service-role uit de shell, `DEMO `-prefix à la de rooktest-`E2E`-conventie) ontgrendelt die klasse. Uitwerking: [plan 29](docs/plans/29-junior-uitwerking-backlog.md) K6. |
| INF-15 | Platform | Web-crash-sweep voor huishoek.app (Playwright) | Next | Could | S | ⏳ | — | **Nieuw (2026-07-07, backlog-doorlichting).** Web-crashes worden nu pas in productie via Sentry gevonden (DialogHost/HUISHOEK-1, zie PLT-10) terwijl de web-build live joins bedient (PLT-7). Spiegel de Android-deeplink-sweep: Playwright laadt de sweep-routes op een lokale web-export en faalt op console-errors/`t-error-boundary`; later in CI. Uitwerking: [plan 29](docs/plans/29-junior-uitwerking-backlog.md) K7. |
...
| INF-4 | Platform | Foutrapportage/monitoring (Sentry) | Next | Should | S | 🔧 | — | **Gewired** (plan [`08`](docs/plans/08-professioneel-hardening.md)): `lib/monitoring.js` env-gated + `ErrorBoundary`. **Sentry-project aangemaakt** (`evdn/huishoek`, EU-region `de.sentry.io`, 2026-06-26); DSN **live als EAS-env** (`EXPO_PUBLIC_SENTRY_DSN` op `@evdns-team/huishoek`, prod/preview/dev, 2026-06-26) + lokaal in `.env`; `metro.config.js` via `getSentryExpoConfig` genereert de source maps + debug-ID's. **Source-map-upload via de EAS↔Sentry-dashboard-integratie** (Expo-UI gekoppeld → EAS uploadt zelf, `SENTRY_DISABLE_AUTO_UPLOAD=true` gezet; geen handmatige `SENTRY_AUTH_TOKEN` nodig). Setup-runbook in [`docs/eas-setup.md`](docs/eas-setup.md). **Sentry bevestigd live in productie (2026-06-30):** ving zijn eerste echte fout (HUISHOEK-1, `release Huishoek@1.0.0`, env `production`) mét onze `context: render`-tag uit de `ErrorBoundary` → leidde direct tot de dialog-web-fix (zie **PLT-10**). Web-ruisfilter toegevoegd in [`lib/monitoring.js`](lib/monitoring.js) (`ignoreErrors: /\.at is not a function/` — scanbots/pre-ES2022-engines). **Edge-kant GEBOUWD (2026-07-07, stream, plan 28 INF-4-mini-plan §2):** pure [`sentryCore.js`](supabase/functions/_shared/sentryCore.js) (DSN-parse + store-event, ratchet 97,6%) + Deno-glue [`sentry.ts`](supabase/functions/_shared/sentry.ts) (`reportEdgeError`: plain fetch, 2s-timeout, fail-silent, geen DSN → no-op) aangeroepen in de catch-paden van `assistant` én `scan-receipt` zonder responses te wijzigen. **Erik-stap:** `supabase secrets set SENTRY_DSN=<dsn>` (bestaand project herbruikbaar) → testfout forceren → event checken. **Rest:** de web-build-frames kwamen nog **geminified** binnen (gs/hs) — web-source-map-upload nog te verifiëren; en een crash uit een **native** cloud-build gesymboliceerd terugzien (EAS↔Sentry-integratie). |
...
| INF-9 | Platform | Edge-hardening `scan-receipt` | Next | Should | S | 🔧 | — | **Gebouwd+gedeployed — incl. fail-closed (statuscorrectie 2026-07-07, code-geverifieerd via [plan 29](docs/plans/29-junior-uitwerking-backlog.md) S4):** de L1-fix uit [plan 17](docs/plans/17-security-remediatie.md) bleek al gebouwd — de rate-limit weigert bij élk niet-happy-pad vóór de Orq-call ([`index.ts:82-126`](supabase/functions/scan-receipt/index.ts)), met getrapte volume-remmen (migr. `0056`/`0057`) + MIME-whitelist + 8MB-cap. **Rest:** happy-path op toestel; een échte euro-/tokencap is optioneel vervolgwerk (eerst voorleggen). |
...
§6 (en verdwijnt hier). Gegroepeerd naar afstand/aard. Voor WEN-1, PLT-4, PLT-9, PLT-2
en AGE-3 staat een bouwbare uitwerking klaar in
[plan 29](docs/plans/29-junior-uitwerking-backlog.md) (F8/F9/F10) — promotie naar §6
blijft de eigenaars-keuze (beslis-agenda plan 29 §6).
...
> PLT-5, PLT-7, PLT-8, PLT-3, DOC-1 en **PLT-4** (2026-07-09) zijn **gepromoot naar §6** —
  > daarom hier weggehaald, conform de §7-regel "verdwijnt hier".
- **Losse items via web delen (netwerkeffect)** (PLT-9) — bv. een boodschappenlijst deelbaar via een
  ...
  Expo Web-host + token-/zichtbaarheidsaanpak. Het **boodschappen-deel is gepromoveerd naar BOO-18**
  (2026-07-09); dit §7-idee blijft voor de overige deelbare dingen. Later uitwerken.
