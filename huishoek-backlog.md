...
| AI-19 | Assistent | AI overal: tool-packs voor planten/huisdieren/voertuigen/tijdlijn/delen | Now | Should | L | ⏳ | AI-17, AI-18 | **Gekozen (2026-07-06, review) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md) golf 1b/2a.** Fase A: 5 read-tools op de bestaande pure logica (plantCare/petCare/vehicleCosts/timeline/reservations), gen-UI via het render.js-vocabulaire. Fase B: 5 additieve HITL-writes (géén financiële boekingen; `hasConflict` verplicht bij reserveren; visibility default household). Let op: kruist het tool-budget (~21) → expliciet tool-search-beslispunt (guidelines §1). Per tool golden-cases + ratchet-groep. |
...
| PLT-8 | Platform | Wachtwoordloze login: e-mail-OTP (native-first) | Next | Should | M | ⏳ | PLT-7 | **§7 → rij (2026-07-06, gekozen) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md) golf 3a.** `signInWithOtp` + 6-cijferige code (géén magic-link-URL: native `detectSessionInUrl` uit, deep-links dekken alleen /join — gevalideerd). Raakt lib/auth.js, welcome.js, join-goLogin; display-name na eerste login. **Erik-actie:** Supabase-dashboard (OTP aan, template, redirect-allowlist). |
| PLT-3 | Platform | Globaal zoeken over modules heen | Next | Should | M | ⏳ | — | **§7 → rij (2026-07-06, gekozen) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md) golf 3c.** RPC `global_search(q)` (SECURITY INVOKER — RLS scopet) over 8 tabellen + pure ranker `lib/searchRank.js` + zoekscherm via Meer/Thuis. Migratienummer: zie plan 27-toewijzing. |
