...
| AI-20 | Assistent | Eval-volwassenheid: multi-turn-cases + NL-toon-judge + Maestro-AI-flow | Now | Should | M | ⏳ | AI-3, AI-18 | **Gekozen (2026-07-06, review) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md) golf 1a.** (1) Golden-schema + runner multi-turn (vervolg-beurt na confirm, choice-reply-opvolging; gemockte tool-results; laatste beurt scoren). (2) LLM-as-judge voor NL-toon (`toneScore` in baseline+gate, gekalibreerde rubric). (3) `.maestro/06-assistent.yaml` rooktest-flow (chat→kaart→confirm→vervolg-beurt). Vóór AI-19 fase B — de gate verbreedt eerst. |
