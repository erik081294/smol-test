...
| MLT-2 | Maaltijden | Recepten + ingrediënten | 2 | Should | M | ✅ | BOO-5 | **Plan 09 — gebouwd; `0016` live**. `recipes`/`recipe_ingredients`, `app/recipe/[id].js`, gekoppeld aan producten/catalogus (`useProducts.suggestFor`). |
...
| PLT-1 | Platform | Notificaties & herinneringen | 2 | Should | M | 🔧 | — | **Plan 05 — trap 1 af; migratie `0018` (`push_tokens`) live (2026-06-20)**. Trap 1 lokaal (`expo-notifications`, `useNotifications`, prefs-scherm) werkt. **Trap 2 remote nu productie-klaar (2026-06-21):** `notify` opgesplitst in pure kern (`core.js`, event-router + handler-registry, 7 units in `tests/notify.test.js`) + impure schil (`index.ts`) met gedeeld-geheim-check (`x-notify-secret`), idempotentie + audit (`push_deliveries`, migratie `0023`), batchen (≤100) en automatische opruiming van dode tokens (`DeviceNotRegistered`). `verify_jwt=false` in `config.toml`. **Rest = flip-on** (vereist Supabase-toegang): migraties pushen, secret zetten, `functions deploy notify`, Database Webhook op `tasks`, 2-account-toesteltest — exact in `docs/notify-setup.md`. |
...
| INF-1 | Platform | Live-Supabase-verificatie + RLS-tests | 1 | Must | S | ✅ | — | Migraties 0004–**0021** live (DB op `0021`); `0016`–`0021` toegepast 2026-06-20 via de Supabase-connector — keuken-loop, kosten-autodelen, push-tokens, inzichten, recurring-`pg_cron`, FK-indexen. Advisor: geen kritieke bevindingen, geen onge-indexeerde FK's meer op de nieuwe tabellen. Units **188 tests / 0 fail** (17 RLS-integratietests lokaal geskipt zonder secrets; groen tegen live DB met secrets). Resteert de handmatige 2-account-rooktest (`VERIFICATIE.md` Stap 3). |
