...
| AI-19 | Assistent | AI overal: tool-packs voor planten/huisdieren/voertuigen/tijdlijn/delen | Now | Should | L | ◐ | AI-17, AI-18 | **Fase A GEBOUWD (2026-07-06) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md).** 5 read-tools live in de code (toolset 11→16, NO_ASSISTANT leeg, 10 golden-cases; edge-deploy volgt met fase B). Fase A was: 5 read-tools op de bestaande pure logica (plantCare/petCare/vehicleCosts/timeline/reservations), gen-UI via het render.js-vocabulaire. Fase B: 5 additieve HITL-writes (géén financiële boekingen; `hasConflict` verplicht bij reserveren; visibility default household). Let op: kruist het tool-budget (~21) → expliciet tool-search-beslispunt (guidelines §1). Per tool golden-cases + ratchet-groep. |
