...
| UX-23 | Platform/UX | Veeg-acties op de Vandaag-focus-taken | 1.6 | Could | S | ⏳ | UX-17 | **Bevinding (UXR-1, 2026-06-23).** De focus-taken op het Thuis-dashboard (`vandaag.js`) zijn afvinkbaar + tikbaar naar detail, maar missen de `SwipeRow`-veegacties die de Taken-tab nu wél heeft (links verwijderen, rechts uitstellen) → inconsistent. Wrap de focus-`TaskRow`s in `SwipeRow` met dezelfde undo-handlers. Klein; geen migratie. |
