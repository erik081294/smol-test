...
| UX-21 | Platform/UX | Bestaande plant bewerken (naam/soort/locatie) | 1.6 | Should | S | ⏳ | UX-18 | **Bevinding (UX-18, 2026-06-23).** Een bestaande plant is nu **niet bewerkbaar**: `/plant/[id]` toont alleen-lezen detail; alleen `/plant/new` heeft een editor (`addPlant`). Daardoor is de verzorgingskaart (afgeleid van soort+locatie) niet aanpasbaar. Voeg een `updatePlant`-flow + zichtbare "Aanpassen"-affordance toe (soort/locatie/naam), net als de huisdier-verzorgingssheet. Geen migratie (kolommen bestaan). |
