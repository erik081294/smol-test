...
| PLT-1 | Platform | Notificaties & herinneringen | Next | Should | M | 🔧 | — | **Plan [`05`](docs/plans/05-notificaties.md).** Trap 1 lokaal werkt; trap 2 remote `notify` productie-klaar (migr. `0018`/`0023` live). **Flip-on gedaan (2026-07-01):** `notify` edge-function **gedeployed** (v1 ACTIVE, `verify_jwt=false`), `NOTIFY_WEBHOOK_SECRET` gezet en de **Database-Webhook** op `tasks` (Insert+Update) live; auth server-side geverifieerd (geldig secret → 200, fout/geen → 401). SEC-5-payloadvalidatie zit erin. **App-kant push-ready gemaakt (2026-07-01):** `expo-notifications`-config-plugin + conditionele `android.googleServicesFile`-hook in [`app.config.js`](app.config.js) (klikt aan zodra `google-services.json` er is; builds zónder blijven werken); `google-services.json`/FCM-sleutel gitignored. **Bevinding aflevertest (2026-07-01):** de lokale dev-client heeft geen FCM → `getExpoPushTokenAsync` faalt (`FirebaseApp failed to initialize`), `push_tokens` blijft leeg; via de EAS-API bevestigd dat `androidFcm: null` voor beide varianten. Notify server-side end-to-end geverifieerd (synthetische toewijzing → `{"processed":1,"sent":0,"skipped":1}` 200; fout secret → 401). **FCM ingericht + push-ready build (2026-07-02):** Firebase-project `huishoek-62492` (Android-app op `app.huishoek`); `google-services.json` als secret EAS file-env `GOOGLE_SERVICES_JSON` (uit git); FCM V1 service-account-sleutel geüpload + gekoppeld aan `app.huishoek` (`googleServiceAccountKeyForFcmV1`, `isLegacy: false`). **EAS dev-build gedraaid** (INF-5: eerste build!) → APK geverifieerd push-ready (`google_app_id`/`gcm_defaultSenderId` van `huishoek-62492` zitten erin). **Rest: alleen nog de on-device tick** — APK installeren → permissie → token in `push_tokens` → alice wijst taak toe aan erik → push landt. Runbook: [`docs/notify-setup.md`](docs/notify-setup.md) §FCM-credentials. |
