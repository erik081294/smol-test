...
| AI-3 | Assistent | Golden-set + evaluators + eval-gate (experiment geparkeerd) | Now | Should | M | ◐ | AI-2 | **Kern gebouwd (2026-07-04):** [`tests/assistant-golden.json`](tests/assistant-golden.json) (29 NL-cases incl. 8× "geen tool") + meta-test die 'm tegen de registry bewaakt; eval-runner [`scripts/assistant-eval.mjs`](scripts/assistant-eval.mjs) (eerste-beurt tool-F1 + args-subset + no-tool, baseline in `assistant-eval-baseline.json`, tolerantie 2pp — **F1 96 / args 100 / geen-tool 100** na de beknopt+suggest_replies-prompt); Orq-judges `huishoek-nl-toon` + `huishoek-groundedness` (drempel ≥70) via MCP aangemaakt. **Geparkeerd op verzoek:** het sonnet-vs-GLM-experiment + schaduwdraai (GLM ook geblokkeerd: Orq's zai-route geeft z.ai-429 zonder eigen key; alternatief = ander GLM-provider-model activeren). **Rest:** judges aan productie-traffic hangen (vergt agent/deployment-route), golden-set uitbreiden vanuit trace-review. |
| AI-4 | Assistent | Chat-persistentie (conversatielijst, hervatten, titels) | Next | Should | M | 🔧 | AI-2 | **Gebouwd (2026-07-04, ronde C):** migratie 0069 (`updated_at`+index, **live**); edge function persisteert beide beurten (user-bericht vóór de LLM-call — vraag nooit kwijt), leest history uit de DB (client stuurt alleen `conversationId`+vraag), deterministische titel (`titleFromMessage`), bumpt `updated_at`; hook laadt gesprekkenlijst + hervatten + nieuw gesprek; gesprekken-sheet in het scherm (history-icoon in de kop). **E2E-bewezen:** vervolgvraag "wanneer moet die af?" beantwoord uit DB-context; 4 rijen + titel geverifieerd. **Device-bevestigd (2026-07-04, moto, licht+donker):** chip-tap → beknopt antwoord + kaart (dubbel-render-bug gevonden en gefixt), gesprekken-sheet (nieuw/hervatten, actief gesprek gemarkeerd), hervat gesprek laadt kaarten én gepersisteerde antwoordopties uit de DB. |
| AI-5 | Assistent | SSE-streaming end-to-end + markdown | Next | Should | M | 🔧 | AI-4 | **Gebouwd (2026-07-04, ronde D).** Server: SSE-route in [`assistant/index.ts`](supabase/functions/assistant/index.ts) (protocol `delta\|tool_status\|tree\|done\|error` in `core.js`, beurt persisteert óók bij client-abort) — **gedeployed (v10)**. Client: [`lib/assistantStream.js`](lib/assistantStream.js) (pure SSE-reducer, ratchet 86%), `useAssistant` streamt via `expo/fetch` met non-streaming-terugval (alléén als de request de server niet bereikte — geen dubbele rate-limit-tel), streamende bubble + tool-statusregel in het scherm. **Markdown:** pure subset-parser [`lib/markdownLite.js`](lib/markdownLite.js) (bold/cursief/code/kopjes/lijsten, streaming-tolerant, ratchet 86%) gerenderd in `AssistantMessageView` — lost de platte `**markdown**` op; `react-native-streamdown` bewust niet (vergt device-spike, zie plan 24-risico). **Rest:** device-rooktest van de stream (licht/donker) + markdown-weergave op toestel. |
...
| AI-7 | Assistent | A2UI-alignment gen-UI (surface/patch, +3 nodes) | Later | Could | M | ⏳ | AI-5 | Ronde F: `assistantUi.js` → surface + patch-op-id + gescheiden data-model (A2UI v0.9 wire-contract, platte tree blijft werken); nodes `progress`/`image`/`chips`. ~~onAction-bridge~~ → vervroegd geland met AI-8 (2026-07-05): [`lib/assistantActions.js`](lib/assistantActions.js) (besluit-whitelist + status-stempeling). |
| AI-8 | Assistent | Write-tools + HITL (multi-edit bevestigingskaart) + skill-file-refactor | Next | Should | L | 🔧 | AI-4 | **Gebouwd (2026-07-05, na industry-onderzoek — ontwerp in [guidelines §1](docs/assistent-architectuur.md)).** (1) **Skill-file per module:** registry opgesplitst naar [`_shared/tools/<moduleKey>.js`](supabase/functions/_shared/tools/index.js) + aggregator; naming hernoemd naar `<moduleKey>_<onderwerp>` (Anthropic-namespacing) — eval-bevestigd: tool-F1 96,4 → **100** na herijk (38 cases). Nieuw Keuken-pack: `maaltijden_weekmenu` + `maaltijden_plannen`. (2) **HITL:** de tool-call ís het voorstel — harness onderschept `kind:'write'`, puur `propose()` → `role='action'`-rij (0068 dekte dit al, geen migratie) → bevestigingskaart mét per-item checkboxes (multi-edit) → `execute` draait **opgeslagen** args (single-shot claim, TTL 1u) → toast + undo (insert-ids). 3 write-tools: taken/boodschappen/maaltijden. Statusmachine [`assistant/actions.js`](supabase/functions/assistant/actions.js) + client-bridge [`lib/assistantActions.js`](lib/assistantActions.js), alles mutatie-bewaakt (87,8–100%), contract-metatest dwingt het pack-contract af. Golden-set +9 cases (write/weekmenu). **Edge function v11 gedeployed via MCP + E2E live-bevestigd (2026-07-05, curl met user-JWT):** write-verzoek → confirm_action-kaart met 2 aanvinkbare items → confirm met deelselectie `[0]` → alléén dat item in `groceries` → dubbeltik = 409 → undo verwijdert de rij → tweede undo = 409. **Rest:** device-rooktest van de kaart-UI zelf (checkboxes/Doen/Niet doen/toast op de moto). |
| AI-10 | Assistent | Assistent overal + mens↔AI-overdracht (briefs, scherm-context, edit-voorstel) | Now | Should | L | 🔧 | AI-8 | **Gebouwd (2026-07-05).** (1) **Module-briefs**: elke skill-file exporteert `<MODULE>_BRIEF`; snapshot toont één regel per actieve module (progressive disclosure; exact vastgepind per pack-test). (2) **Scherm-context**: client stuurt `screen` (moduleKey) mee → snapshot-regel "aanwijzing, geen beperking". (3) **Assistent overal**: [`assistantProvider`](lib/assistantProvider.js) (één gespreksstate app-breed — lost het AI-6-remount-restpunt op) + [`AssistantSheet`](lib/AssistantSheet.js)-overlay + **AI-first FAB's** op taken/vandaag/maaltijden (bewuste herziening plan 23 §5: FAB → chat met focus; "Zelf invoeren" = uitwijk naar de klassieke editor). (4) **Mens↔AI-overdracht**: "Bewerken" op de bevestigingskaart → generieke edit-sheet ([`EDITABLE_FIELDS`](lib/assistantActions.js), registry-contract-getest + propose-roundtrip) → `decision:'edit'` hervalideert via dezelfde pure `propose()`, status blijft pending, `edited_by_user` in audit-spoor → `openProposalsNote` geeft de AI de actuele versie in de volgende beurt. **Edge v12/v13 gedeployed (CLI) + E2E live bewezen:** voorstel → edit (Testmelk→Testhavermelk 2 pakken) → AI kent de bewerking → confirm voert de bewerkte args uit → undo; kapotte edit → nette 400. Eval-gate 100/100/100 (38 cases, productie-tokenbudget — 400-token-reasoning-artefact gefixt). **Rest:** device-rooktest sheet/FAB/edit-flow; FAB-uitrol naar overige modules zodra die write-tools krijgen. |
| AI-9 | Assistent | Geheugen v1 (pgvector hybrid + async extractie + beheer-UI) | Later | Could | L | ⏳ | AI-4 | Ronde H: migratie 0071 `assistant_memories` (vector + 'dutch' tsvector, user/household-scope-RLS) + hybrid-RRF-RPC; `remember_fact` (HITL) + async extractor (aparte deployment, dedupe); beheer-scherm; memory-gebruik zichtbaar in chat. |
