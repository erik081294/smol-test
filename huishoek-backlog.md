...
| ARCH-1 | Fundament | Gedeelde entity-editor (formValidation + useEntityForm) | Now | Should | M | ◐ | — | **Fundament gebouwd** (architectuur-review 2026-06-26): pure [`lib/formValidation.js`](lib/formValidation.js) (ratchet 92,1%) + dunne [`lib/useEntityForm.js`](lib/useEntityForm.js); `visibilityRule` in [`lib/visibility.js`](lib/visibility.js). [`app/expense/[id].js`](app/expense/[id].js) omgezet als referentie. **✅ Smoke-test 2026-06-26 (moto):** nieuw + bewerken opslaan werken; de gedeelde `runRules` toont omschrijving-/bedrag-/deelnemers-meldingen en blokkeert een niet-sluitende exact-split (inline cue) — gedragsneutrale conversie bevestigd. **Rest:** plant/pet/vehicle/task incrementeel migreren, nieuwe editors volledig (values+errors). Contract: [`docs/architectuur.md`](docs/architectuur.md). |
| ARCH-2 | Fundament | Capability-interface voor overzichten | Next | Should | M | ⏳ | ARCH-1 | Vandaag/Notificaties vragen elke ingeschakelde module om z'n samenvatting i.p.v. zusterhooks te importeren — lost de enige hook-naar-hook-koppeling op ([`useNotifications`](lib/useNotifications.js) → `useTasks`/`useMealPlan`/`usePantry`). [`docs/architectuur.md`](docs/architectuur.md). |
| ARCH-3 | Fundament | Module-gating in de datalaag | Next | Could | S | ⏳ | — | Hooks/schermen respecteren `effectiveModules()` ([`lib/modules.js`](lib/modules.js)): laad geen data van een uitgezette module. |
| ARCH-4 | Fundament | i18n/ui per domein-namespace splitsen | Later | Could | S | ⏳ | — | [`lib/i18n.js`](lib/i18n.js) (~1165 rgl) / [`lib/ui.js`](lib/ui.js) (~1209 rgl) opdelen om merge-wrijving te dempen naarmate er modules bijkomen. Puur opruimen, gedragsneutraal. |
...
| INF-1 | Platform | Live-Supabase-verificatie + RLS-tests | Now | Must | S | 🔧 | — | Migraties live **t/m `0057`** (volledige repo-set toegepast; MCP `list_migrations` 2026-06-26); kern-RLS 13/13 (`docs/rls-connector-check.sql`). **Volledige live-RLS-suite groen — 729 pass / 0 skip / 0 fail (2026-06-26)**, incl. de nieuwe timeline-/invite-isolatiecases; de INF-12 batch/backoff-fix houdt de auth-rate-limit weg. **Rest:** 2-account-rooktest (`VERIFICATIE.md`). |
...
| INF-4 | Platform | Foutrapportage/monitoring (Sentry) | Next | Should | S | 🔧 | — | **Gewired** (plan [`08`](docs/plans/08-professioneel-hardening.md)): `lib/monitoring.js` env-gated + `ErrorBoundary`. **Sentry-project aangemaakt** (`evdn/huishoek`, EU-region `de.sentry.io`, 2026-06-26); DSN → `EXPO_PUBLIC_SENTRY_DSN` (`.env`/EAS-env), source-map-upload bedraad: config-plugin met org/project/url + `metro.config.js` via `getSentryExpoConfig`, auth via `SENTRY_AUTH_TOKEN` (EAS-env). Setup-runbook in [`docs/eas-setup.md`](docs/eas-setup.md). **Rest:** `SENTRY_AUTH_TOKEN` als EAS-env + eerste build die source maps uploadt; crash op toestel verifiëren in Sentry. |
...
| INF-10 | Platform | DB-advisor-hardening | Next | Could | S | 🔧 | — | B4 af (migr. `0024`); **M1 GEBOUWD+LIVE** (migr. `0042`–`0044`: anon/PUBLIC-EXECUTE ingetrokken op user-facing DEFINER-RPC's, `authenticated` + RLS-helpers behouden). **Migr. `0058` geschreven (2026-06-26, nog niet live):** trekt anon/PUBLIC-EXECUTE in op de RLS-helpers (`is_member`/`is_owner`/`in_subgroup`/`can_view` — `authenticated` behouden) + de trigger-fns (`handle_new_user`/`check_subgroup_household`/`cleanup_vehicle_resource`). Reverseert bewust de 0044-uitzondering op gecorrigeerde premisse; vraagt akkoord vóór live (MCP `apply_migration`) + RLS-suite/advisor-hercheck. **Open:** B5 pg_trgm uit `public`, B6 leaked-password (dashboard). [plan 17](docs/plans/17-security-remediatie.md). |
