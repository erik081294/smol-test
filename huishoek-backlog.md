...
| PLT-4 | Platform | Export als deelbare tekst/CSV | Next | Could | S | ⏳ | — | **Gepromoveerd uit §7 (besluit 2026-07-09, D11).** Pure formatters in `lib/export.js` (boodschappenlijst als deelbare tekst, saldo als CSV; unit-getest + ratchet-groep) + het bestaande `Share.share`-patroon ([`huishouden.js`](app/(tabs)/huishouden.js#L128)); ingang als gelabelde drawer-actie (UX-42-contract). Uitwerking: [plan 29](docs/plans/29-junior-uitwerking-backlog.md) F9. |
