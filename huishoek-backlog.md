...
| SCH-4 | Schoonmaak | Zelf custom schoonmaaktaken én roosters opstellen | Next | Should | M | 🔧 | SCH-1, SCH-2 | **Gebouwd (2026-06-29, via UXR-9/[plan 20](docs/plans/20-schoonmaak-teardown.md)).** Alle drie de wensen: (1) **zelf een rooster samenstellen** — pure [`buildCustomSchedule`](lib/cleaningTemplates.js) (deelt de kern met `planTemplate`, unit-getest, ratchet `cleaningTemplates` **76,0%**) + een "Zelf samenstellen"-modus in de opstel-sheet ([`schoonmaak.js`](app/(tabs)/schoonmaak.js)): kies zones (sjabloon-zones + bestaande huishoud-zones) en per zone een cadans (wekelijks/om-de-week/maandelijks); (2) **strak onderscheid** — per-zone "Taak toevoegen" (losse taak) vs. de footer-knop "Rooster opstellen" (heel schema); (3) **rooster bekijken per week/maand** via een **deeplink** "Rooster bekijken" → `/(tabs)/taken?cleaning=1&scope=week`: nieuwe zone-as in [`applyTaskFilters`](lib/agenda.js) (`cleaningOnly`/`zoneId`, unit-getest, ratchet `agenda` **92,5%**) + route-param-parsing + een "Schoonmaak"-sectie in de taakfilter-sheet ([`taken.js`](app/(tabs)/taken.js)). **Rest:** device-rooktest (custom rooster opstellen → taken verschijnen; "Rooster bekijken" filtert correct in Taken). |
| PLA-10 | Planten | Meer controle over de verzorgingstaken | Next | Should | M | 🔧 | PLA-3 | **Eerste stap gebouwd (2026-06-29, via UXR-6/[plan 21](docs/plans/21-zorg-teardown.md)).** **Statuscorrectie:** een verzorgingstaak was vanaf het detail níét te openen — plant- én huisdier-detail gaven de taakrijen geen `onPress`, dus `TaskRow` viel via `taskHref` terug op ditzélfde detail (dode tik). **Nu:** de taakrijen openen de taak-editor (interval/frequentie/weekdagen/herhaal-einde) op [`app/plant/[id].js`](app/plant/%5Bid%5D.js) én [`app/pet/[id].js`](app/pet/%5Bid%5D.js) (parity). **+ "Taak toevoegen"** op de plant → `/task/new?plant=<id>`: de taak-editor kreeg een `plant`-passthrough (category `plant` + `plant_id` in de payload, symmetrisch met `zone`). **Open (in [plan 21](docs/plans/21-zorg-teardown.md)):** expliciet pauzeren/hervatten (staat-keuze → mogelijk migratie), plant-visibility overnemen bij een handmatige taak, evt. een per-plant care-overzicht (huisdier-`openCareSheet`-stijl). **Rest:** device-rooktest. |
