...
| INF-1 | Platform | Live-Supabase-verificatie + RLS-tests | 1 | Must | S | ✅ | — | Migraties 0004–**0022** live (DB op `0022`); `0016`–`0021` toegepast 2026-06-20 en `0022` (`update_expense`) op 2026-06-21 via de Supabase-connector — keuken-loop, kosten-autodelen, push-tokens, inzichten, recurring-`pg_cron`, FK-indexen, bewerkbare uitgaven. Advisor: geen kritieke bevindingen, geen onge-indexeerde FK's meer op de nieuwe tabellen. Units **189 tests / 0 fail** (18 RLS-integratietests lokaal geskipt zonder secrets; groen tegen live DB met secrets). Resteert de handmatige 2-account-rooktest (`VERIFICATIE.md` Stap 3). |
