...
| FND-5 | Fundament | Wisselen tussen huishoudens (lid van meerdere tegelijk) | Next | Should | M | ⏳ | FND-1 | **Toestelfeedback 2026-06-28:** iemand kan in meerdere huishoudens zitten (eigen gezin + bv. ouderlijk huis); er is nog geen manier om actief te wisselen. De datalaag kent al een **actief huishouden** ([`useHousehold().activeId`](lib/household.js)), maar de UI gaat uit van één. **Verken/ontwerp:** lidmaatschap van meerdere huishoudens, een huishouden-switcher (kop of instellingen) en wat "actief" raakt (hooks/queries scopen al op `activeId` → realtime opnieuw subscriben bij wissel; cache legen zoals bij logout). Eerst ontwerp, dan bouw. |
...
| BOO-10 | Boodschappen | Bonnen bewerkbaar maken | Next | Could | M | 🔧 | BOO-2 | **Gebouwd** (`update_purchase`-RPC, migr. `0033`) + editor + "Bewerken"-tak ([`app/purchase/[id].js`](app/purchase/[id].js)). **Entry-point opgelost (2026-06-27):** nieuwe bonnenlijst [`app/purchases.js`](app/purchases.js) (winkel · datum · #producten · totaal) → tik = `/purchase/<id>`; bereikbaar via de "Bonnen"-link op Boodschappen, met "bon invoeren" vanaf de lijst/lege staat. **Rest:** device-rooktest (lijst → bon openen → bewerken/opslaan). |
| BOO-13 | Boodschappen | Producteditor + "opslaan in catalogus?"-prompt bij nieuw item | Next | Should | M | 🔧 | BOO-5 | **Gebouwd (2026-06-27).** Migr. `0061` (`products.emoji`) + `0062` (`products.photo_path` + private bucket `product-images`, household-RLS — spiegelt recepten/0034), **live** via MCP, additief/nullable. Producteditor [`app/product/edit.js`](app/product/edit.js) op de gedeelde entity-editor ([`useEntityForm`](lib/useEntityForm.js) + [`Editor`](lib/ui.js)): naam, schap ([`CATEGORIES`](lib/groceryCatalog.js)), standaard-eenheid, **emoji** ([`EmojiPicker`](lib/ui.js)) én **foto** ([`offerImagePicker`](lib/photoPicker.js) → [`uploadPhoto`](lib/photoStorage.js), getoond via gecachete signed URL). [`useProducts`](lib/useProducts.js) `updateProduct`/`setProductPhoto`/`clearProductPhoto` schrijven naar de gedeelde rij → **huishouden-breed**. **Werkt voor álle catalogusproducten**: tik op een product in de Catalogus → editor (een bundel-/zoek-item wordt eerst aangemaakt = "opslaan", dán bewerken). **Prompt:** na een nieuw item toevoegen vraagt de Catalogus "even aankleden?" → editor. Bewerkte emoji/foto tonen in "Eerder gekozen" ([`ProductImageView`](lib/ProductImageView.js) prefereert foto → emoji → schap-emoji). **Rest:** device-rooktest (editor opslaan, foto-upload + signed-URL-render, prompt-flow) + UX-copy fijnslijpen. **Vindbaarheid (toestelfeedback 2026-06-28):** Erik vond de producteditor niet — de enige ingang is nu Catalogus → tik op een product (+ de "even aankleden?"-prompt ná een nieuw item). Neem in de rooktest mee of dit ontdekbaar genoeg is (overweeg een expliciete "product toevoegen/bewerken"-actie in de Catalogus). |
| BOO-14 | Boodschappen | Lijst krijgt meer schermruimte (verken-item, toestelfeedback 2026-06-28) | Next | Should | M | ⏳ | — | **Probleem (Erik, op toestel):** de lijst zelf is klein doordat er veel chrome bóven de `SectionList` stapelt in [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js): `ScreenHeader` (titel **+ subtitle**), de toevoegbalk, een vaste "Catalogus openen"-knop + bonnen-link, en de "Misschien weer nodig"-carousel — pas daaronder begint de lijst. **Voorgestelde oplossingen (verken, kies/combineer):** (1) kop laten inklappen bij scrollen (compacte titel, subtitle weg/samengevoegd); (2) "Catalogus openen" + "Bonnen" uit de vaste rij halen → header-actie/overflow (sluit op **UX-42**, header-rechts opschonen); (3) "Misschien weer nodig" inklapbaar of alleen bij korte/lege lijst tonen, óf als eerste lijst-sectie i.p.v. een vaste band erboven; (4) toevoegbalk + lijst als prioriteit, de rest secundair. Eerst meten wat hoeveel hoogte kost, dán snoeien. |
| BOO-15 | Boodschappen | Zoekbalk: na productkeuze sluiten + clean herfocussen voor het volgende item | Next | Should | S | ⏳ | — | **Probleem (Erik):** kies je een product uit de zoek-dropdown, dan blijft de getypte tekst staan en blijft de dropdown open; je moet handmatig terug om verder te zoeken. **Wens:** na het toevoegen van een product → tekst legen, dropdown sluiten **en de focus teruggeven aan de balk** (`TextInput`-ref → `focus()`, balk heeft al `blurOnSubmit={false}`) zodat je meteen het volgende item typt. Nu doen `addLinked`/`addCustom` wel `setText('')` maar de dropdown-`Stepper` (`setCatalogCount`) niet — een tik op de productregel zou moeten toevoegen + legen + herfocussen (de stepper blijft voor aantal-fijnregelen). [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js). |
| BOO-16 | Boodschappen | Zoekbalk: snelle wis-knop (×) om de invoer te legen | Next | Should | S | ⏳ | — | **Probleem (Erik):** na "zes tomaten" aanklikken moet je terug naar de balk en met backspace het veld leegmaken — traag. **Wens:** een ×/wis-knop in of naast de balk die in één tik `setText('')` doet (en de focus houdt), zichtbaar zodra er tekst staat. Klein; sluit aan op BOO-15 (herfocus). [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js#L217). |
| BOO-17 | Boodschappen | Distincte visuele feedback bij afvinken/verwijderen i.p.v. stil verdwijnen | Next | Should | S | ⏳ | — | **Probleem (Erik):** afvinken én verwijderen laten het item nu gewoon verdwijnen (`animateNextLayout` + toast in [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js)) — geen prettige, duidelijke bevestiging, en de twee acties voelen identiek. **Wens:** een **onderscheidende** animatie per actie — bv. afvinken: rij kort als afgevinkt tonen (check kleurt door + strikethrough) vóór hij inklapt, eventueel een `Celebrate`-burst (bestaat al in [`lib/ui.js`](lib/ui.js), UX-44); verwijderen: een duidelijk wég-gebaar (wegvegen/uitfaden). Doel: je ziet wát er gebeurde, en afvinken ≠ verwijderen. Sluit op UX-44 (feedback-timing). |
...
| SCH-4 | Schoonmaak | Zelf custom schoonmaaktaken én roosters opstellen | Next | Should | M | ⏳ | SCH-1, SCH-2 | **Toestelfeedback 2026-06-28.** Nu zijn roosters vaste sjablonen ([`CLEANING_TEMPLATES`](lib/cleaningTemplates.js) → `planTemplate`); een losse taak kan al per zone ([`schoonmaak.js`](app/(tabs)/schoonmaak.js) → `/task/new?zone=`). **Wensen:** (1) zélf een rooster samenstellen (eigen taken + frequentie, niet alleen een sjabloon kiezen); (2) **strak onderscheid** tussen losse schoonmaaktaken en het rooster opstellen; (3) het rooster per **week** en **maand** kunnen bekijken — mogelijk via een **deeplink met schoonmaakfilter naar de Taken-tab** i.p.v. een eigen weergave (hergebruik de bestaande taken-filters). Uitwerken in de teardown **UXR-9**. |
| PLA-10 | Planten | Meer controle over de verzorgingstaken | Next | Should | M | ⏳ | PLA-3 | **Toestelfeedback 2026-06-28.** De verzorgingstaken worden nu regelgebaseerd gegenereerd ([`lib/plantCare.js`](lib/plantCare.js) → terugkerende `tasks`). **Wens:** per plant meer grip — een taak **bewerken/uitzetten**, het **interval aanpassen** of een eigen verzorgingstaak toevoegen, i.p.v. alleen het afgeleide schema. Overlapt met de Zorg-teardown **UXR-6** ("verzorging bewerken"); daar concreet maken. |
...
| HUI-2 | Huisdieren | Eigen diersoort toevoegen | Later | Should | S | 🔧 | HUI-1 | **Gebouwd (2026-06-27).** Migr. `0063` (`pets.species_label`, additief/nullable, **live** via MCP). Bij soort "Anders" verschijnt een vrij "Anders, namelijk…"-veld ([`app/pet/[id].js`](app/pet/[id].js)); [`usePets.addPet`](lib/usePets.js) schrijft `species_label` (alleen bij `type:'anders'`). Pure [`speciesLabel(pet)`](lib/petCare.js) (eigen label wint, anders de vaste soort) — unit-getest, mutatie-ratchet `petCare` 68%. Detail toont het eigen label. **Rest:** device-rooktest. |
| VTG-1 | Voertuigen | Voertuig + onderhoudsschema (module) | Later | Should | M | 🔧 | — | **Gebouwd (PR #51 `feat/voertuigen-pro`):** `vehicles` + `lib/vehicleCare.js`-sjablonen → checklist-`tasks` (cat. `voertuig`); plannen op datum én km. **Device 2026-06-25:** module + voertuig "Cliootje" (Renault Clio · 2019 · JZP70N, RDW-data ingevuld) renderen ✓, voertuig-taken op Vandaag/Taken. **Device 2026-06-26 (moto, live dev-client):** voertuig-detail rendert rijk — RDW-velden ("Merk en model opgehaald bij de RDW", "Rood · Hatchback · APK t/m 13-03-2028"), km-stand 63000, kosten ("€110,11 / maand") + prijs per km, en "Delen via Samen". **Rest:** onderhoudschecklist/km-flow-interactie op toestel. ✅ **Statusreconciliatie gedaan (2026-06-27):** VTG-2/3/4 staan nu op 🔧 — UI op toestel gevuld en de code bevestigt dat kosten/historie, RDW-lookup en delen-via-Samen gebouwd zijn; rest is device-rooktest. |
| VTG-2 | Voertuigen | Onderhoud plannen: kosten & historie | Later | Should | M | 🔧 | VTG-1, KOS-1 | **Code-bevestigd gebouwd (statusreconciliatie 2026-06-27):** [`lib/vehicleCosts.js`](lib/vehicleCosts.js) (`vehicleCostSummary` → kosten/maand + prijs/km) getoond op het voertuig-detail; onderhoud loggen (datum/km/kosten/notitie, optioneel als gedeelde uitgave → KOS-1) + historie via `vehicle_log` ([`lib/vehicleTimeline.js`](lib/vehicleTimeline.js)) in [`app/vehicle/[id].js`](app/vehicle/%5Bid%5D.js). **Rest:** device-rooktest. |
| VTG-3 | Voertuigen | Kenteken → auto-type via RDW | Later | Should | S | 🔧 | VTG-1 | **Gebouwd** (`lib/rdw.js` normaliseren/valideren/mappen + niet-blokkerende lookup; editor vult merk/model/type + verrijking). **2026-06-27:** een kenteken-**wijziging** ververst nu de afgeleide velden (merk/model/type/**bouwjaar** uit `firstRegistration` + verrijking incl. **APK**) i.p.v. de oude auto te laten staan; op het openen van een bestaand voertuig worden alleen lege velden gevuld (`appliedPlate`-baseline). **Rest:** device-rooktest; statusreconciliatie VTG-2/VTG-4 (UI aanwezig, eigen device-check). |
| VTG-4 | Voertuigen | Voertuig delen via de Samen/Delen-module (auto = default) | Later | Should | M | 🔧 | VTG-1, AUT-1 | **Code-bevestigd gebouwd (statusreconciliatie 2026-06-27):** voertuig = gedeelde resource (`resource_id`-koppeling, default aan via de `shared`-toggle) + prijs per km ([`lib/vehicleSharing.js`](lib/vehicleSharing.js) `parseRatePerKm`/`formatRatePerKm`) in [`app/vehicle/[id].js`](app/vehicle/%5Bid%5D.js). **Rest:** device-rooktest (delen aan/uit + reserveren). |
...
| PLT-10 | Platform | Web-mobile: swipen + camera werkend krijgen (crasht nu) | Next | Should | M | ⏳ | — | **Toestelfeedback 2026-06-28:** op web-mobile crasht het vegen van taken (`SwipeRow`, react-native-gesture-handler) én de camera/foto-flow ([`lib/photoPicker.js`](lib/photoPicker.js)) met de generieke "er is onverwacht iets misgegaan" (de `ErrorBoundary` uit LRN-1 vangt het). **Verken:** reproduceren op web-mobile, isoleren wélke module throwt, en een web-veilige variant of feature-detectie/fallback bouwen zodat de schermen niet omvallen. |
...
| INF-13 | Platform | Auth: e-mailbevestiging linkt op productie naar huishoek.app (nu localhost) | Next | Should | S | ⏳ | — | **Toestelfeedback 2026-06-28.** De bevestigingsmail ("Check je mail om je account te bevestigen", [`auth.signup.confirm`](lib/i18n.js)) wijst naar `localhost`. **Oorzaak:** [`auth.signUp`](lib/auth.js#L48) geeft geen `emailRedirectTo` mee → Supabase gebruikt de **Site URL + redirect-allowlist** uit het dashboard, die nog op localhost staat. **Fix:** productie Site URL + redirect-allowlist op `https://huishoek.app` zetten (jouw Supabase-dashboard) en/of `emailRedirectTo: 'https://huishoek.app/…'` in `signUp` meegeven. Sluit op de web-host van PLT-7. |
...
| UXR-9 | UX-review | Ontleding: Schoonmaak (samen strak zetten) | Next | Should | M | ⏳ | SCH-4 | **Klaargezet op verzoek (2026-06-28).** Verkennende teardown van [`schoonmaak.js`](app/(tabs)/schoonmaak.js) + de zones/sjablonen-laag, samen door te lopen. Kern-input: het **onderscheid taken vs. rooster opstellen** strak trekken, **custom roosters** (SCH-4), en rooster-weergave week/maand (eigen view óf deeplink+schoonmaakfilter naar Taken). Lens/werkwijze: [plan 14](docs/plans/14-ux-module-teardown.md). |
