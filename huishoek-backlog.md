...
| INF-6 | Platform | i18n-fundament | 2 | Should | S | 🔧 | — | **Laag gelegd + alle 8 tab-schermen gemigreerd:** `lib/i18n.js` (`t(key, vars)` met `{var}`-interpolatie, zichtbare key-fallback, `setLang`/`registerDict`, simpele `plural`), units `tests/i18n.test.js` (6, groen). Alle copy (headers, filters, empty states, toasts, Alerts, a11y-labels) op Vandaag/Taken/Meer/Agenda/Kosten/Planten/Boodschappen/Schoonmaak + `TaskRow` via `t(...)`. **Regel:** nieuwe code gebruikt `t(...)`. **Rest:** editors (taak/uitgave/plant), auth-welkom, onboarding en huishouden incrementeel; later `expo-localization` voor locale-detectie. Plan 06. |
