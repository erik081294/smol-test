...
| AI-16 | Assistent | A2UI: industry-leading interactieve gen-UI-componenten (grafiek/rooster/recept) | Next | Should | L | ⏳ | AI-7, AI-15 | **Idee → rij (2026-07-05, Erik).** De huidige gen-UI-nodes zijn functioneel maar matig; doel = superflexibele, mooie, **interactieve** componenten waarmee de AI stuk-voor-stuk industry-leading UI bouwt — een grafiek, een rooster/schema, een rijke recept-kaart. Bouwt op AI-7 (A2UI v0.9 surface/patch-model): (1) nieuwe node-types mét interactie (`chart`, `schedule`, verrijkte `recipe`), elk = server-side `render*`-helper + client-renderer + interactie via de onAction-bridge; (2) de multiple-choice-beslis-UI als eigen node-type i.p.v. alleen chips (AskUserQuestion-kaart met varianten — sluit op AI-15-(3a)); (3) componenten die live updaten via het patch-model (porties bijstellen → ingrediënten herrekenen). Lat: interactief, toegankelijk, thema-veilig. Aanpak: max +3 node-types per ronde (guidelines §9), elk met eval + device-verificatie. |
| AI-9 | Assistent | Geheugen v1 (pgvector hybrid + async extractie + beheer-UI) | Later | Could | L | ⏳ | AI-4 | Ronde H: migratie `assistant_memories` (**eerstvolgend vrij nummer — 0071/0072/0073 zijn bezet (0073 = `save_recipes`, AI-14), dus 0074**; vector + 'dutch' tsvector, user/household-scope-RLS) + hybrid-RRF-RPC; `remember_fact` (HITL) + async extractor (aparte deployment, dedupe); beheer-scherm; memory-gebruik zichtbaar in chat. |
