...
| INF-1 | Platform | Live-Supabase-verificatie + RLS-tests | 1 | Must | S | 🔧 | — | Migraties 0004–**0024** live (DB op `0024`); `0016`–`0021` toegepast 2026-06-20, `0022` (`update_expense`) en op 2026-06-21 ook **`0023` (`push_deliveries`, PLT-1 trap 2) + `0024` (`function_search_path`, security-hardening B4)** via de Supabase-connector — geverifieerd via `list_migrations` + advisors. Units **196 tests / 0 fail** (18 RLS-integratietests lokaal geskipt zonder secrets; groen tegen live DB met secrets). Resteert: live-RLS-tests draaien mét secrets + de handmatige 2-account-rooktest (`VERIFICATIE.md` Stap 3). Zie `docs/vervolgplan-2026-06-21.md`. |
