...
| INF-3 | Platform | E2E-tests (Maestro) | Next | Should | M | 🔧 | — | **Error-bewuste rooktest** ([`docs/rooktest.md`](docs/rooktest.md)): `npm run rooktest` = Maestro-flows + logcat-oordeel + exit-code. 5 flows in `.maestro/` (crash-sweep + 4 behavior), selectors op `t-*`-id's (`lib/ui.js` + tabs). **Rest:** flows `00`–`03` op toestel kalibreren (`04` al geverifieerd) — **kalibratie loopt (2026-07-01)** met `maestro studio` op het USB-toestel. |
...
| SEC-5 | Security | `notify`-payload valideren vóór deploy | Next | Should | S | ✅ | PLT-1 | **GEBOUWD (code+units, ratchet 80,2%):** `notify/core.js` recipientId-guard + `clampBody`; titel al server-side getemplatet. **Live meegedeployed met `notify` (2026-07-01, zie PLT-1)** — de validatie draait nu in de gedeployede edge-function. M4, [plan 17](docs/plans/17-security-remediatie.md). |
