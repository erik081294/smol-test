...
| PLT-1 | Platform | Notificaties & herinneringen | 2 | Should | M | 🔧 | — | **Plan 05 — trap 1 gebouwd; migratie `0018` (`push_tokens`) live (2026-06-20)**. Trap 1 lokaal (`expo-notifications`, `useNotifications`, prefs-scherm) werkt. Units `lib/notifications.js` uitgebreid. **Rest = volgende ronde (gekozen 2026-06-21): trap 2 remote push live** — Edge Function `notify` deployen (kan via de Supabase-connector `deploy_edge_function`) + Database Webhook + toestel-push-test. Ontsluit het proactieve vliegwiel (diner vanavond / "bijna over datum" / taak-toewijzing). |
| PLT-6 | Platform | Activiteiten-/wijzigingenfeed | 2 | Could | M | ⏳ | — | **Volgende ronde (gekozen 2026-06-21).** "Tim vinkte 'stofzuigen' af", "Erik voegde melk toe". Nu goedkoop: de voltooiingen-log `task_completions` (0012) + realtime bestaan al. Aanpak: óf afleiden uit bestaande logs/`created_at`/realtime-events, óf een lichte `activity`-tabel (insert-triggers per module). Gescopet via `can_view`/`is_member`. Pure formatter `lib/activity.js` (event → NL-regel) + een feed-scherm/Thuis-kaart. |
