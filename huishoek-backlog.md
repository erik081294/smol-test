...
| INF-9 | Platform | Edge-hardening `scan-receipt` | 2 | Should | S | ⏳ | — | Audit S-M4: de LLM-proxy `supabase/functions/scan-receipt/` heeft geen rate-limiting (elk lid kan onbeperkt dure Orq.ai-calls afvuren → kosten/DoS) en geen MIME-whitelist. Toevoegen: per-gebruiker rate-limit + `mimeType`-whitelist, daarna `functions deploy scan-receipt`. |
