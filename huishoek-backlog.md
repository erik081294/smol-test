...
| TKN-3 | Taken/UX | Schaalbare filterbediening (categorie/module · persoon · subgroep · status) | 2 | Should | M | ⏳ | TKN-1 | **Deel van de redesign — zie §7.7.** De huidige filters (horizontale categorie-`Chip`-rij + open/done-toggle in `app/(tabs)/taken.js`) schalen slecht: bij **veel modules/categorieën** wordt de chip-rij een lange horizontale scroll (lage ontdekbaarheid), en bij **veel huishoudleden** is er nog géén "toegewezen aan"-filter. **Doel:** één compacte, schaalbare filter-affordance náást de tijdscope — een **"Filter"-knop met teller** die een **`BottomSheet`** opent (hergebruik `lib/ui.js`) met gegroepeerde keuzes: **categorie/module**, **toegewezen aan** (lid-avatars, `AvatarSelect`), **zichtbaarheid/subgroep** (`VisibilityPicker`) en **status** (open/af). Actieve filters tonen als **verwijderbare chips** onder de header + **"wis alles"**; selectie blijft combineerbaar met de scope (TKN-1) en blijft per sessie bewaard. Pure filter-/telkern in `lib/` (unit-getest), web- en a11y-veilig (48dp-targets, labels). Géén migratie. |
| UX-8 | Platform/UX | Opstart-/wachtscherm i.p.v. flitsende "Huishouden aanmaken" | 1.5 | Should | S | ⏳ | — | **Bug + redesign.** Bij inloggen ziet een gebruiker die al lid is van een huishouden soms kort het **onboarding-scherm ("Huishouden aanmaken")** opflitsen vóór de app laadt. **Oorzaak:** de redirect-`useEffect` in `app/_layout.js` stuurt naar `/onboarding` zodra `households.length === 0` is, terwijl dat óók de begintoestand is vóór de eerste fetch klaar is; de guard (`authLoading || (session && hhLoading)`) dekt de race niet altijd (`hhLoading` kan al `false` zijn met een lege lijst tussen sessie-herstel en de huishouden-fetch). **Fix (datalaag):** onderscheid "**nog niet geladen**" van "**echt geen huishouden**" — bv. een aparte `ready`/`hasFetched`-vlag in `lib/household.js` zodat de onboarding-redirect pas vuurt ná een voltooide fetch; tot dan in de wacht-/laadtoestand blijven. **Fix (UI):** vervang de kale `ActivityIndicator` op `colors.forest` (`app/_layout.js` regel ~59-65) door een **net wachtscherm met illustratie** (`lib/illustrations.js`, thema-/dark-mode-bewust) — merkgroet + zachte beweging (`lib/motion.js`, respecteer `prefersReducedMotion`), zodat het opstarten rustig en af voelt i.p.v. een spinner. Géén migratie. |
...
| UX-10 | Platform/UX | "Vorige"-lintje linksboven (terug-knop mét herkomst-naam) | 2 | Should | M | ⏳ | UX-1 | **Navigatie-helderheid (vervolg op STR-2).** Detailschermen sluiten nu met een **naamloze ✕ rechtsboven** (`ModalHeader` in `lib/ui.js`) en de editors met "Annuleer" — geen van beide zegt **wáár je heen gaat**. **Doel:** een iOS-achtig **"vorige"-lintje linksboven**: een chevron `‹` + **de naam van het scherm/tab waar terug-navigeren naartoe gaat** ("‹ Boodschappen", "‹ Planten"), zodat de herkomst expliciet is en de navigatie heel duidelijk wordt. **Aanpak:** nieuwe gedeelde **`BackBar`** (of een `back`-variant/-prop op `ModalHeader`) in `lib/ui.js`, doorgevoerd op de ~18 detailschermen (`app/plant/[id]`, `app/purchase/[id]`, `app/product/[id]`, `app/expense/[id]`, `app/recipe/[id]`, `app/resource/[id]`, `app/recurring-expense/[id]`, `app/task/[id]`, `app/catalog`, `app/kosten-inzichten`, `app/herinneringen`, `app/beeldstijl`…). **Herkomst-naam bepalen:** primair een **per-route ouder-tab-mapping** (bv. plant-detail → "Planten", bon/product → "Boodschappen", uitgave → "Kosten") als betrouwbare basis; verfijn waar nuttig met de **echte vorige route** uit de Expo Router-stack (`useNavigation`/`getState`). **Deep-link/koude start** (geen echte "vorige"): val terug op de logische ouder-tab. **a11y + web-veilig:** `accessibilityRole="button"`, label "Terug naar {scherm}", chevron-icoon links, 48dp-target; labels via `t(...)` (i18n). Vervangt/aanvult de ✕-sluitvorm; behoudt de Annuleer·titel·Bewaar-vorm voor editors. Géén migratie. |
| UX-11 | Platform/UX | FAB kleiner + label "wat je toevoegt" naast de + | 2 | Should | S | ⏳ | UX-1 | **Kleine, brede polish.** De `FAB` (`lib/ui.js`) staat op **7 schermen** maar rendert overal als een **kale ronde +** (58×58, icon 28) met alleen een `accessibilityLabel` — de component **ondersteunt al een `label`-prop** (extended-modus met tekst naast het icoon), maar geen scherm geeft die door. **Doel:** (a) de + **iets kleiner** maken (bv. 58→~52, icon 28→~24) zodat 'ie minder dominant is; (b) **tekst naast de +** die zegt **wát je toevoegt** — zet de bestaande extended-modus aan met een kort `label` per scherm: **Taak** (vandaag/taken/agenda), **Plant** (planten), **Uitgave** (kosten), **Voorraad** (voorraad), **Item** (delen) — labels via `t(...)`. De volledige zin blijft in `accessibilityLabel`. **Let op:** een extended FAB is breder → controleer dat 'ie niet botst met lijst-inhoud en netjes boven de inset zweeft (UX-5 al gefixt); op smalle schermen evt. inkorten. Sluit aan op STR-5 ("één duidelijke primaire actie"). Géén migratie. |
| UX-12 | Platform/UX | Back vanuit een via-"Meer"-geopende tab gaat naar Home i.p.v. Meer | 1.5 | Should | S | ⏳ | UX-2 | **Navigatie-bug.** Open je via **Meer** een niet-primaire tab (bv. **Planten**/**Voorraad**) — `router.push('/(tabs)/${m.route}')` in `app/(tabs)/meer.js` naar een tab-route met `href:null` (`app/(tabs)/_layout.js`) — en druk je dan de **systeem-back** (Android-hardware/gebaar), dan spring je naar **Home/Vandaag** i.p.v. terug naar **Meer**. **Oorzaak:** de `Tabs`-navigator gebruikt de standaard **`backBehavior: 'firstRoute'`**, dus back gaat naar de **initiële tab** (Vandaag), niet naar de vorig bezochte tab; de tab-router houdt geen bezoekgeschiedenis bij. **Fix-richtingen (kies bewust):** (a) **`backBehavior="history"`** op de `Tabs` → back volgt de bezochte volgorde (terug naar Meer) — eenvoudig, maar raakt **álle** tab-back-navigatie; (b) niet-primaire modules vanuit Meer als een **echte stack-push met eigen terug-knop** openen i.p.v. een tab-switch (sluit aan op het **"vorige"-lintje UX-10**) — gerichter; (c) een eigen back-handler die "kwam via Meer" detecteert en daarheen terugkeert. **(b) heeft de voorkeur** (consistent met UX-10). Verifieer op **toestel** (Android hardware-back **én** gebaar) en web. Géén migratie. |
