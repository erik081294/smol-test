...
| INF-1 | Platform | Live-Supabase-verificatie + RLS-tests | 1 | Must | S | 🔧 | — | **Alle migraties `0001`–`0036` live (DB op `0036`).** Geverifieerd via `list_migrations` op 2026-06-22 (36 versies; `0016`+ onder timestamp-versies, namen matchen de repo 1-op-1; `0035` = `plant_diary_note`, `0036` = `home_layout`). Laatst toegevoegd na 0025: catalogus-/scan-hardening (`0026`–`0031`), realtime-delete (`0032`), `update_purchase` (`0033`), recept-bucket (`0034`), plant-notitie (`0035`), home-layout (`0036`). Kern-RLS + RPC's bewezen via **`docs/rls-connector-check.sql`** (13/13, rol/JWT-impersonatie + rollback, geen secrets nodig). Units **296 (278 pass / 0 fail / 18 skip)** — de 18 RLS-integratietests skippen lokaal zonder secrets. Resteert: de volledige JS-RLS-suite mét secrets + egress + de handmatige 2-account-rooktest (`VERIFICATIE.md` Stap 3). Verificatierecept zonder secrets: `docs/rls-connector-check.sql`. |
...
| INF-8 | Platform | Realtime-primitief & scoping | 1.5 | Should | M | 🔧 | — | **C1+C2 af (2026-06-21).** De gedupliceerde realtime-/channel-boilerplate uit 7 hooks geëxtraheerd naar `lib/useRealtimeReload.js` (C1); de brede `expense_shares`/`purchase_items`-subscripties filteren nu op `household_id` (C2 — migr. `0025` voegde `expense_shares.household_id` toe, `purchase_items` had die al). Lost audit A-H1/A-H2 + deels P-H1/P-H3 op. **C3 gebouwd (2026-06-22):** incrementeel patchen uit `payload.new/.old` i.p.v. full refetch. Pure kern `lib/realtimePatch.js` (`comparatorFromOrder` + `applyRealtimePatch` voor INSERT/UPDATE/DELETE met hersorteren; `null` = fallback naar full reload) + units `tests/realtimePatch.test.js`. `useRealtimeReload` kreeg een optionele `onChange(payload)`; `useCollection` geeft die mee **alleen bij platte `select '*'`-collecties** (join-select-hooks — `useTasks`/`useExpenses`/`usePurchases`/`useMealPlan`/`useTaskCompletions` — blijven bewust reload-on-event, want de embedded relatie komt niet mee in de payload). **C4 gebouwd (2026-06-22): kanalen gebundeld tot één household-channel** — gedeelde hub `lib/realtimeHub.js` (`createRealtimeHub(client, { schedule })`, injecteerbaar → puur unit-getest in `tests/realtimeHub.test.js`, 7 cases). Alle hooks van één huishouden delen nu **één** Supabase-kanaal i.p.v. een eigen kanaal per hook; per unieke (tabel, filter) is er precies **één** postgres_changes-listener die fan-out't naar alle hook-callbacks (dedup: Home's boodschappen-widget + de Boodschappen-tab delen één server-abonnement). Rebuild is gedebounced (mount-burst van de Vandaag-grid → 1 kanaal) en gebeurt alléén als de set unieke bronnen wijzigt. Lost de "hook-storm" op de widget-grid op. `useRealtimeReload` is hier transparant op overgezet (API ongewijzigd); de singleton leeft in `useRealtimeReload.js`. **Rest:** realtime-patch + de gebundelde subscriptie op een toestel/2-richtingen bevestigen. |
...
| INF-10 | Platform | DB-advisor-hardening | 1.5 | Could | S | 🔧 | — | **B4 af (migr. `0024`):** vaste `search_path` op `enable_module_rls`/`search_catalog`. **Advisor-stand 2026-06-22 (`get_advisors security`):** **B5** `pg_trgm` in `public` (WARN) + **B6** leaked-password-protection uit (WARN) staan **nog open** (bevestigd). **Nieuw gezien (nog niet aangepakt):** (a) `anon`/`authenticated` mogen alle `SECURITY DEFINER`-functies aanroepen (lint 0028/0029, WARN — geldt voor de RLS-helpers `is_member`/`can_view`/… én de RPC's; de RPC's doen hun eigen lidmaatschapscheck, dus dit is grotendeels by-design maar de `EXECUTE` voor `anon` mag strakker); (b) 3× `rls_enabled_no_policy` (INFO) op `catalog_sync_state`/`push_deliveries`/`receipt_scans` — **bewust deny-all** (alleen via service-role/`security definer`-RPC/cron geschreven, geen client-toegang), geen lek. **Rest:** `pg_trgm` uit `public` naar een eigen schema (B5 — let op de afhankelijke trigram-index op `catalog_products.search`) + leaked-password-protection aanzetten (B6 — Auth-dashboard, geen connector-tool) + evt. `EXECUTE` op de helpers intrekken voor `anon`. |
| PERF-1 | Platform | Query-vensters & bulk-RPC | 2 | Could | M | 🔧 | INF-8 | **Veilige helft gebouwd (2026-06-22).** Audit P-H2: ruime `.limit(2000)` op de voltooiingen-log (`lib/useTaskCompletions.js`) én op `expenses` (`lib/useExpenses.js`), beide op `… desc` zodat de meest recente behouden blijven — begrenst de worst-case zonder in de praktijk iets te veranderen (2000 = jaren huishouden). `tasks`/`groceries` bewust **niet** geblanket-limiteerd (de "af"-filter toont voltooide taken). **Rest (open, bewust buiten dit pakket):** exacte all-time-aggregaten (saldo/eerlijkheid) bij >2000 rijen → server-side aggregaat-RPC; P-H4 bulk-RPC voor bon→voorraad i.p.v. rij-voor-rij. |
| PERF-2 | Platform/UX | Waargenomen snelheid: instant tab-wissel (geen laad-flits) | 2 | Should | M | 🔧 | INF-8 | **Gebouwd (2026-06-22, richting a+b).** (a) **Client-cache met stale-while-revalidate**: lichte in-memory store `lib/dataCache.js` (gekeyd op `tabel:householdId`, household-scoping = geen cross-household-lek, `clearCache()` op sign-out in `lib/auth.js`) + units `tests/dataCache.test.js`. Alle `useCollection`-hooks (taken/boodschappen/voorraad/delen/producten/planten) én de vier custom-loader-hooks (`useExpenses`/`usePurchases`/`useMealPlan`/`useTaskCompletions`) seeden hun begintoestand uit de cache (`loading=false` bij een gecachete — ook lege — lijst) en schrijven na elke fetch/patch terug → een herbezochte tab toont meteen data i.p.v. `ListSkeleton`; de bestaande skelet-conditie `loading && items.length===0` lost vanzelf op (geen schermwijzigingen nodig). Huishouden-/venster-wissel re-seedt uit de juiste cache (geen stale-flits van het vorige huishouden). (b) **`freezeOnBlur: true`** op de `Tabs` (`app/(tabs)/_layout.js`) houdt een bezochte tab gemount + render-bevroren. Verweven met INF-8 C3 (incrementele realtime-patch werkt de cache bij). **(c) skeleton-drempel + (d) data-hoisting bewust niet gedaan** — (a)+(b) lossen de dominante hervisite-flits al op. **Rest:** soepelheid op web + Android-emulator/toestel bevestigen (let op: `freezeOnBlur` mag geen lege/bevroren tab achterlaten bij terugkeer). **Doel: van tab wisselen moet *instant* voelen.** Nu opent bij (vrijwel) elke tab-wissel kort een **laad-skelet** vóór de inhoud staat — dat wil de gebruiker weg. **Oorzaak:** elk tab-scherm haalt zijn data via hooks (`useCollection`, `lib/useTasks.js`, `useExpenses`, `usePantry`, …) die **bij elke mount opnieuw** starten met `loading=true` + een **lege lijst** en vers van het netwerk fetchen; er is **geen cache tussen mounts** en de `Tabs` gebruiken **geen `freezeOnBlur`/state-behoud**, dus een tab-wissel toont telkens de `ListSkeleton` (`app/(tabs)/taken.js`, `boodschappen.js`, `voorraad.js`, `delen.js`, `maaltijden.js`, …) terwijl dezelfde data opnieuw geladen wordt. **Fix-richtingen (kies bewust, gelaagd):** (a) **client-cache met stale-while-revalidate** — toon meteen de eerder geladen data en ververs stil op de achtergrond (React Query/SWR óf een eigen lichte in-memory store per `household_id`+tabel, sluit aan op `useRealtimeReload`/INF-8); geen skeleton meer bij hervisite. (b) **`freezeOnBlur`** op de `Tabs` (`app/(tabs)/_layout.js`) zodat een bezochte tab niet hard remount/refetcht. (c) **skeleton-drempel** (~150–200 ms): toon het laad-skelet pas als de fetch écht langer duurt, zodat een snelle hervisite geen flits geeft. (d) gedeelde huishoud-data **hoisten** naar een provider zodat tabs dezelfde in-memory data delen i.p.v. elk apart te laden. **Aanpak:** gelaagd en meetbaar — eerst (a)+(c) als grootste winst per inspanning, daarna (b)/(d) waar nodig; respecteer realtime-waarheid (cache mag stale tonen, moet snel bijtrekken). Bouwt voort op INF-8 (realtime-primitief) en raakt PERF-1 (query-vensters). Verifiëren op dev build/web met echte navigatie, niet blind bouwen. Géén migratie. |
...
| TKN-2 | Taken/UX | Jaarweergave — activiteit-heatmap | 3 | Could | M | 🔧 | TKN-1 | **Gebouwd (2026-06-22, gekozen richting a — activiteit-heatmap).** De Jaar-scope van Taken toont een GitHub-achtig bijdrage-raster van taakvoltooiingen over een kalenderjaar, gevoed uit de voltooiingen-log `task_completions` (migratie 0012 — géén nieuwe migratie). **Pure kern** `lib/yearHeatmap.js` (`countsByDay` met lid-/categorie-filter, `levelFor` schaal-op-max, `yearGrid` weken×weekdagen + rand-weken + maandlabels, `yearSummary` totaal/actieve dagen/langste+huidige reeks/drukste weekdag/beste dag) + 15 units `tests/yearHeatmap.test.js` (lokale dag-bucketing TZ-onafhankelijk, streaks, toekomstig/leeg jaar). **Visueel** `lib/YearHeatmap.js` (horizontaal-scrollbaar raster, forest-alpha-tinten light/dark, Ma/Wo/Vr-labels, maandkoppen, legenda Minder→Meer, per dag een gelabelde 48dp-tikbare cel met ring op de selectie, vandaag-accent). **Container** `lib/YearActivity.js` — eigen `useTaskCompletions` (**lazy-mount: de realtime-subscriptie leeft alléén in de Jaar-scope**, PERF/INF-8), jaar-stepper (geen toekomst), **lid-selector** (`AvatarSelect`, bij >1 lid) **+ categorie-filter** (chips; beide gaan AND in `countsByDay`), samenvatting-tegels, tik-op-dag-callout (selectie blijft staan bij een filterwissel, telling updatet live) en een uitnodigende lege staat. **Verfijnd (evaluatieronde):** kleur-ramp één keer per render i.p.v. ~366× parsen, geen wrapper-`View` per cel, toekomstige dagen niet-interactief, ongebruikte `bestDay` uit `yearSummary` gehaald. Gewired als vierde scope in `app/(tabs)/taken.js` (de TKN-3-filterbalk is in deze scope bewust verborgen — die stuurt open/geplande taken, niet de log). i18n-sleutels onder `tasks.year.*` (alleen `nl`). 313 units groen, lint 0 errors. **Rest:** rendering + horizontale scroll + realtime-bijwerken op web/toestel bevestigen. **Oorspronkelijk onderzoek/uitvraag (achtergrond):** Bij de jaar-scope (TKN-1) was nog onbekend wat de meest waardevolle weergave is. **Uit te vragen + verkennen:** wil de gebruiker (a) een **activiteit-heatmap** (GitHub-bijdrage-grid van voltooiingen per dag over het jaar, voedbaar uit de voltooiingen-log `task_completions`, migratie 0012); (b) een **maand-voor-maand-overzicht** (12 mini-maandjes / dichtheid per maand); (c) een **seizoens-/jaarplanner** voor terugkerende & seizoensklussen (leunt op KLU-3 `months`/`seasonalChores()` — "wat staat dit jaar nog te gebeuren"); of (d) een combinatie. Bepaal scope + databron ná de uitvraag; pas dán een eigen TKN-rij voor de bouw. Mogelijk geen migratie (afgeleid uit `task_completions`/`tasks`). |
