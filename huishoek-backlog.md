...
| BOO-14 | Boodschappen | Lijst krijgt meer schermruimte (verken-item, toestelfeedback 2026-06-28) | Next | Should | M | ◐ | — | **Eerste stap gebouwd (2026-06-28):** de vaste "Catalogus openen"-knop + losse bonnen-link zijn samengevoegd tot één compacte rij (catalogus + bonnen naast elkaar), wat een regel hoogte boven de lijst scheelt; beide blijven duidelijk benoemd/bereikbaar. **Open (verken in UXR-9-stijl, kies/combineer):** (1) kop laten inklappen bij scrollen (compacte titel, subtitle weg/samengevoegd); (2) catalogus/bonnen naar header-actie (sluit op **UX-42**); (3) "Misschien weer nodig" inklapbaar of alleen bij korte/lege lijst, óf als eerste lijst-sectie. Eerst meten wat hoeveel hoogte kost, dán verder snoeien. [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js). |
| BOO-15 | Boodschappen | Zoekbalk: na productkeuze sluiten + clean herfocussen voor het volgende item | Next | Should | S | 🔧 | — | **Gebouwd (2026-06-28).** Tik op een zoekresultaat (`pickCatalog`) voegt +1 toe en roept `clearAndRefocus()` aan: veld legen → dropdown sluit (q leeg) → focus terug op de `TextInput` (`inputRef.current.focus()`, balk heeft `blurOnSubmit={false}`). Submit/`addCustom` idem. De `Stepper` in de rij blijft voor aantal-fijnregelen (laat de balk juist staan). [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js). **Rest:** device-rooktest. |
| BOO-16 | Boodschappen | Zoekbalk: snelle wis-knop (×) om de invoer te legen | Next | Should | S | 🔧 | — | **Gebouwd (2026-06-28).** Een wis-knop (`close`-icoon) in de zoekbalk, zichtbaar zodra er tekst staat; één tik → `clearAndRefocus()` (legen + focus behouden). Nieuwe i18n-key `common.clear`. [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js#L217). **Rest:** device-rooktest. |
| BOO-17 | Boodschappen | Distincte visuele feedback bij afvinken/verwijderen i.p.v. stil verdwijnen | Next | Should | S | 🔧 | — | **Gebouwd (2026-06-28).** Afvinken geeft nu een **success-haptiek** + korte bevestigingstoast (`groceries.checkedFeedback`, 1,5s, alleen bij áfvinken — niet bij terugzetten); verwijderen een **lichte tik** (`haptics.tapLight`) náást de bestaande undo-toast. Zo voelen/zien afvinken en verwijderen verschillend. Een verdere in-rij-animatie (rij kort als afgevinkt vóór inklappen / `Celebrate`-burst) blijft een optie voor UX-44. [`app/(tabs)/boodschappen.js`](app/(tabs)/boodschappen.js). **Rest:** device-rooktest. |
...
| SCH-4 | Schoonmaak | Zelf custom schoonmaaktaken én roosters opstellen | Next | Should | M | ⏳ | SCH-1, SCH-2 | **Toestelfeedback 2026-06-28.** Nu zijn roosters vaste sjablonen ([`CLEANING_TEMPLATES`](lib/cleaningTemplates.js) → `planTemplate`); een losse taak kan al per zone ([`schoonmaak.js`](app/(tabs)/schoonmaak.js) → `/task/new?zone=`). **Wensen:** (1) zélf een rooster samenstellen (eigen taken + frequentie, niet alleen een sjabloon kiezen); (2) **strak onderscheid** tussen losse schoonmaaktaken en het rooster opstellen; (3) het rooster per **week** en **maand** kunnen bekijken — mogelijk via een **deeplink met schoonmaakfilter naar de Taken-tab** i.p.v. een eigen weergave (hergebruik de bestaande taken-filters). Uitwerken in de teardown **UXR-9** ([plan 20](docs/plans/20-schoonmaak-teardown.md), klaargezet 2026-06-28: huidige stand + open beslissingen). |
| PLA-10 | Planten | Meer controle over de verzorgingstaken | Next | Should | M | ⏳ | PLA-3 | **Toestelfeedback 2026-06-28.** De verzorgingstaken worden regelgebaseerd gegenereerd ([`lib/plantCare.js`](lib/plantCare.js) → terugkerende `tasks`). **Bevinding (2026-06-28):** een verzorgingstaak ís al volledig bewerkbaar — vanaf het plantdetail ([`app/plant/[id].js`](app/plant/%5Bid%5D.js)) openen de taakrijen de taak-editor (interval/frequentie/weekdagen/einde). Wat ontbreekt is **per-plant grip op één plek**: snel uitzetten/pauzeren of het interval aanpassen zónder per taak de editor in te duiken, en een eigen verzorgingstaak toevoegen. **Bewust ontwerp-eerst** (geen migratie nodig voor "bewerken"; "pauzeren-en-hervatten" vraagt wél een keuze over staat). Concreet maken in de Zorg-teardown **UXR-6** ("verzorging bewerken"). |
...
| UXR-9 | UX-review | Ontleding: Schoonmaak (samen strak zetten) | Next | Should | M | ⏳ | SCH-4 | **Klaargezet op verzoek (2026-06-28) → [plan 20](docs/plans/20-schoonmaak-teardown.md):** huidige stand code-geverifieerd (zone-gedragen module, losse taak per zone kan al, rooster = vast sjabloon, Taken filtert nog niet op schoonmaak), de SCH-4-wensen, en de **open beslissingen** (schoonmaaktaak = `zone_id` of categorie? eigen weergave vs. deeplink naar Taken? custom rooster = terugkerende taken zonder migratie of een benoemd rooster-object? zone-beheer in de UI?). Klaar om samen door te lopen; levert dan concrete `SCH-NN`/`UX-NN`-rijen. Lens/werkwijze: [plan 14](docs/plans/14-ux-module-teardown.md). |
