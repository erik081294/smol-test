...
| BOO-13 | Boodschappen | Producteditor + "opslaan in catalogus?"-prompt bij nieuw item | Next | Should | M | ⏳ | BOO-5 | **Wens (Erik):** voegt iemand een item toe dat nog niet in de catalogus staat, toon dan een schermpje — bv. *"'Komkommer' staat nog niet in jullie catalogus. Wil je 'm opslaan zodat jij en je huishouden 'm voortaan sneller terugvindt?"* (UX-copy nog aanscherpen, duidelijker). **Ja** → de gebruiker landt in een **producteditor** (hergebruik de gedeelde entity-editor: [`useEntityForm`](lib/useEntityForm.js) + [`formValidation`](lib/formValidation.js), vgl. [`docs/architectuur.md`](docs/architectuur.md)) waar **naam**, **afbeelding** ([`photoPicker`](lib/photoPicker.js)/[`productImage`](lib/productImage.js)), **emoji**, **categorie**/schap ([`groceryCatalog.js`](lib/groceryCatalog.js) `CATEGORIES`), **eenheid** en overige relevante velden in te stellen zijn. **De editor moet voor álle catalogusproducten bestaan** (ook bestaande, niet alleen net-nieuwe) — entry-point vanuit catalogus/productdetail. **Huishouden-breed:** een aanpassing schrijft naar de household-`products` (BOO-5, `is_member`-RLS) en geldt dus voor het hele huishouden. **Verken:** een bundel-/OFF-product bewerken = een household-`products`-override aanmaken (de gebundelde [`groceryCatalog.js`](lib/groceryCatalog.js) + de globale OFF-catalogus zijn read-only referentiedata; zie [`useCatalog.js`](lib/useCatalog.js)/[`useProducts.js`](lib/useProducts.js)); waarschijnlijk een lichte additieve migratie voor extra productvelden (emoji/afbeelding-url). |
