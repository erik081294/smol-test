...
| UX-3 | Platform/UX | Editor-flow & bewerkbare records | 2 | Should | M | ✅ | STR-3 | **Gebouwd (UX-sweep, PR #6, 2026-06-21).** Gedeelde `Editor`-scaffold (vaste Bewaar-knop) + `Collapsible` + gedeelde `DateStepper` in `lib/ui.js`; vaste veldvolgorde *wat→wie→wanneer→details→delen→verwijderen*; "Delen met" ingeklapt (`VisibilityPicker collapsible`, strings naar i18n). **Bewerkbare uitgaven** (`update_expense`-RPC, migratie `0022` live) + recept-ingrediënten inline bewerkbaar. Uitgave-datumveld; recurring → undo-toast; reserveren "Hele dag". Conventie in `DESIGN.md`. 189 tests groen. |
| UX-4 | Platform/UX | Dark mode afmaken | 2 | Could | M | ⏳ | UX-3 | Scaffold staat er al: `beeldstijl`-scherm + `lib/themePrefs.js` (`mode: systeem/licht/donker`, persistent) en **themebare** illustraties (`lib/illustrations.js`, palet-tokens). **Rest:** een theme-context die `lib/theme.js` `colors.*` runtime omschakelt (donker palet toevoegen), `userInterfaceStyle` loszetten, en de banner "binnenkort" weghalen. Eén centrale token-laag → schermen erven het automatisch (geen rauwe kleuren in `app/`). **Bewust uitgesteld (2026-06-21):** dit is een theme-context-migratie over ~40 bestanden / 248 `colors.*`-gebruiken die hier níét runtime te verifiëren is (risico op een stil half-donker scherm). Opgepakt op een dev build / web-sessie waar systeem/licht/donker over álle schermen + illustraties visueel na te lopen is — niet blind gebouwd. |
| UX-5 | Platform/UX | Tabbalk/toast veilige onderrand (edge-to-edge insets) | 1.5 | Should | S | ⏳ | — | **Bug (Android-app + mobiel web): de tabbalk valt onder de systeem-navigatieknoppen** (terug/home/recent bij 3-knops, of de gebaarbalk). **Oorzaak:** sinds Expo SDK 52+ is **edge-to-edge standaard aan** op Android (in SDK 56 niet meer uit te zetten) → de app tekent áchter de transparante systeembalken, en de tabbalk in `app/(tabs)/_layout.js` heeft een **harde `height: 86` zonder `paddingBottom`/safe-area-inset**. Een vaste hoogte overschrijft de automatische inset-correctie van React Navigation, dus de balkinhoud zit op `bottom:0` — precies onder de knoppen. Zelfde fout in de toast (`lib/toast.js`, vaste `BOTTOM_OFFSET = 96`). **Gewenst gedrag (zoals andere apps het cross-device doen): edge-to-edge mét window-insets** — de balkinhoud pal bóven de navigatie-controls, het inset-gebied eronder als witruimte in de balkkleur (`colors.surface`); de controls blijven in beeld (géén immersive/verbergen). Patroon van Apple's *Safe Area Layout Guide* (home-indicator ~34pt) en Google's *WindowInsets*/`enableEdgeToEdge`. **Fix:** `useSafeAreaInsets()` (`react-native-safe-area-context`, al in de app) → `tabBarStyle.height = 86 + insets.bottom` + `paddingBottom: insets.bottom`; toast-offset = `insets.bottom +` tabbalkhoogte. iOS (home-indicator) profiteert mee. Klein, gelokaliseerd; geen migratie. |
...
> + **STR-4** PhotoPicker-extractie. **UX-4** (dark mode) was óók ingepland maar bewust
> uitgesteld: het is een ~40-bestanden theme-context-migratie die hier niet runtime te
> verifiëren is — apart op te pakken op een dev build (zie UX-4-noot). Bewust níét nu:
> AI-assistent over eigen data (AI-1) — geparkeerd voor een latere ronde.
...
- **Tabbalk veilige onderrand** (UX-5) — de tabbalk valt op Android (app + mobiel web) onder de
  systeem-navigatieknoppen: edge-to-edge zonder window-insets. Fix via `useSafeAreaInsets()` zodat
  de balk pal bóven de controls zit met witruimte eronder. Raakt elk scherm. Zie §6.
