...
| AI-19 | Assistent | AI overal: tool-packs voor planten/huisdieren/voertuigen/tijdlijn/delen | Now | Should | L | ◐ | AI-17, AI-18 | **Fase A + B GEBOUWD (2026-07-06) → [plan 27](docs/plans/27-ontwikkelprogramma-juli.md).** 10 tools (5 read + 5 HITL-write) — de assistent dekt en bedient alle datamodules. Writes: additief, géén financiële boekingen, conflictcheck bij reserveren, tz-correct via client-offset; undo-whitelist +5; 20 golden-cases. Toolset 16→21 (≤25-metatest; tool-search bewust uitgesteld). **Rest:** edge-deploy (nieuwe bundel-bestanden!), eval-gate-run (ORQ-key), device-verificatie, **fase C: pack-ratchets 76–82% → ≥85** (rest = vooral fouttekst-StringLiterals, zelfde klasse als maaltijden). Fase A was: 5 read-tools op de bestaande pure logica (plantCare/petCare/vehicleCosts/timeline/reservations), gen-UI via het render.js-vocabulaire. Fase B: 5 additieve HITL-writes (géén financiële boekingen; `hasConflict` verplicht bij reserveren; visibility default household). Let op: kruist het tool-budget (~21) → expliciet tool-search-beslispunt (guidelines §1). Per tool golden-cases + ratchet-groep. |
...
| UX-45 | Platform/UX | Design-systeem v2 op toestel verifiëren + illustratie-stage per module-tint | Next | Should | S | 🔧 | UX-1, UX-9 | **Gebouwd & live (2026-07-09, PR #133+#134):** nieuw palet, merk-letters (UX-9), module-tint per module (canonieke bron `colorToken` in [`modules.js`](lib/modules.js) → widget-tegel én `ScreenHeader module=`), vullende `ProgressRing`. `DESIGN.md` herschreven op de échte tokennamen. **Rest:** (1) **device-rooktest** — `type.body` 15→16 en `caption` 12→13 volgen de guide maar kunnen in krappe rijen afkappen; fontlaadtijd en de donkere HomeHero niet op toestel gezien. (2) illustratie-stage draait nog op de gedeelde `forestTint` i.p.v. de module-soft-tint. |
