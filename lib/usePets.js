...
import { deleteDiaryEntryWithCover } from './entityDiary';
...
export function usePets() {
  ...
  const addPet = async ({
    name, type, speciesLabel, birthDate, chipNumber, vetName, notes,
    ...
    const rows = await mutate(
      supabase.from('pets').insert({
        ...
        name, type,
        // Eigen soort-label alleen zinvol bij type 'anders' (HUI-2).
        species_label: type === 'anders' ? (speciesLabel?.trim() || null) : null,
  ...
  return {
    pets: c.items, loading: c.loading, error: c.error, reload: c.reload,
...
export async function deletePetLog({ entry, pet }) {
  return deleteDiaryEntryWithCover({
    entry,
    parentCover: pet?.photo_path ?? null,
    removeObject: (path) => supabase.storage.from(PET_BUCKET).remove([path]).catch(() => {}),
    deleteRow: (id) => mutate(supabase.from('pet_log').delete().eq('id', id), { context: 'tijdlijn-post verwijderen' }),
    fetchRemaining: () => run(
    ...
    setCover: (next) => mutate(supabase.from('pets').update({ photo_path: next }).eq('id', entry.pet_id), { context: 'cover bijwerken' }),
  });
}
