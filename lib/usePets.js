...
import { PET_BUCKET, diaryPhotoPath, contentTypeForExt } from './petPhoto';
...
export async function addPetPhoto({ householdId, petId, userId, asset, note = null, weightGrams = null }) {
  ...
  const path = diaryPhotoPath(householdId, petId, key, asset.ext);
  const { error } = await supabase.storage.from(PET_BUCKET).upload(
...
export async function signedPhotoUrl(path, expiresIn = 3600) {
  if (!path) return null;
  const { data } = await supabase.storage.from(PET_BUCKET).createSignedUrl(path, expiresIn);
  return data?.signedUrl ?? null;
}

// Hook: zet een opslagpad om naar een toonbare (signed) URL. Herresolved als het pad
// verandert; null zolang er geen foto is. `refreshKey` forceert een verse URL.
