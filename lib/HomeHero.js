...
export function HomeHero({ householdName, householdEmoji, greeting, firstName, progress, remaining, loading = false, onPress }) {
  ...
  // Ring → Taken (UX-22): de cirkel léést als tikbaar; nu krijgt die affordance ook
  // betekenis i.p.v. een dode knop. Zonder onPress blijft 'ie een statusindicator.
  const ring = (
    ...
    >
      ...
      {onPress ? (
        <Pressable onPress={onPress} accessibilityRole="button" accessibilityLabel={a11y}
          accessibilityHint={t('today.progress.hint')} hitSlop={8}
          style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1 })}>
          {ring}
        </Pressable>
      ) : (
        <View accessible accessibilityLabel={a11y}>{ring}</View>
      )}
    </View>
  );
}
