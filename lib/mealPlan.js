...
// Aantal eters van een geplande maaltijd: mee-etende huishoudleden + gasten van buiten.
// Ontbrekende velden tellen als nul (een oude entry zonder eter-info → 0).
//   { eater_ids: ['a','b'], extra_eaters: 1 } -> 3
export function eaterCount(entry = {}) {
  const members = Array.isArray(entry.eater_ids) ? entry.eater_ids.length : 0;
  const guests = Number(entry.extra_eaters) || 0;
  return members + guests;
}

// Voorgestelde porties voor een maaltijd: het aantal eters, of `fallback` als er (nog)
// niemand is aangevinkt. Zo koppelt de UI servings standaard aan "wie eet mee", terwijl
// je er handmatig van af kunt wijken.
//   ({eater_ids:['a','b']}, 2) -> 2 · ({}, 2) -> 2 · ({extra_eaters:3}, 2) -> 3
export function defaultServings(entry = {}, fallback = 2) {
  const n = eaterCount(entry);
