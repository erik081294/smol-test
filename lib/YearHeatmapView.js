...
import { todayColumn, heatmapScrollX } from './yearHeatmap';
...
export function YearHeatmap({ grid, onSelectDay, selectedKey }) {
  ...
  // Bij het lopende jaar staat het raster anders helemaal links op 1 januari; we scrollen
  // na de eerste meting (onLayout) naar de week van vandaag zodat die in beeld staat.
  // Voor een afgelopen/toekomstig jaar is er geen today-kolom (-1) → geen scroll, blijft
  // links. Deps zijn primitieven die alleen bij een jaar-/datawissel veranderen, dus dit
  // vecht niet met handmatig scrollen of een dag-selectie.
  const scrollRef = useRef(null);
  const [viewportWidth, setViewportWidth] = useState(0);
  const todayCol = useMemo(() => todayColumn(grid), [grid]);
  useEffect(() => {
    const x = heatmapScrollX({
      col: todayCol, viewportWidth, gridWidth,
      colWidth: COL_W, labelWidth: LABEL_W, padRight: space.lg,
    });
    if (x > 0) scrollRef.current?.scrollTo({ x, animated: false });
  }, [todayCol, viewportWidth, gridWidth]);
