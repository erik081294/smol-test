...
export function useGroceries() {
  ...
  // Live spiegel van de lijst + een per-naam-keten: twee snelle "+"-tikken op hetzelfde
  // product zagen anders allebei "niets gevonden → create" (check-then-act op een niet-
  // optimistische create) en maakten een dubbele regel i.p.v. samen te voegen (review P4).
  // We serialiseren per genormaliseerde naam en lezen bij elke stap de VERSE lijst (ref,
  // door de realtime-echo bijgewerkt) zodat de tweede tik de zojuist gemaakte rij als
  // update ziet. (Volledig atomair zou een upsert-RPC vergen — een migratie; dit dekt de
  // praktijk-race af zonder DB-wijziging, conform de review-aanbeveling.)
  const itemsRef = useRef(c.items);
  itemsRef.current = c.items;
  const chains = useRef(new Map());
  const serialize = (norm, fn) => {
    const prev = chains.current.get(norm) ?? Promise.resolve();
    const result = prev.then(fn, fn); // ná de vorige op op deze naam, ook als die faalde
    const settled = result.catch(() => {}); // de keten mag niet breken op één fout
    chains.current.set(norm, settled);
    settled.then(() => { if (chains.current.get(norm) === settled) chains.current.delete(norm); });
    return result;
  };
  const findOpen = (norm) => itemsRef.current.find((i) => !i.checked && normalize(i.name) === norm);
