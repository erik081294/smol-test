...
// Toonbare soortnaam (HUI-2): bij type 'anders' wint een eigen `species_label`
// ("Anders, namelijk…"); anders de vaste soortnaam. Een leeg/whitespace-label valt
// terug op de vaste naam. Puur, los testbaar.
/** @param {{ type?: string, species_label?: string }} [pet] */
export function speciesLabel(pet) {
  const custom = pet?.species_label?.trim();
  if (pet?.type === 'anders' && custom) return custom;
  return petType(pet?.type).label;
}
...
// Hoort deze (bestaande) taak bij dit verzorgings-template? Een verzorgingstaak heet
// "<template-titel> — <naam>" (zie buildCareTasks), dus we matchen op die titel-prefix.
// Puur zodat de detail-schermen de diff niet ongetest in de schil hoeven te doen
// (schendt anders afspraak 1 van docs/architectuur.md).
/**
 * @param {{ title?: string | null } | null | undefined} task
 * @param {{ title: string }} template
 * @returns {boolean}
 */
export function isCareTaskFor(task, template) {
  return !!task?.title?.startsWith(`${template.title} — `);
}

// Welke template-keys hebben al minstens één gekoppelde taak? (op titel-prefix gematcht)
/**
 * @param {Array<{ key: string, title: string }>} templates
 * @param {Array<{ id?: any, title?: string | null }>} tasks
 * @returns {string[]}
 */
export function presentCareKeys(templates, tasks) {
  return templates
    .filter((tpl) => tasks.some((task) => isCareTaskFor(task, tpl)))
    .map((tpl) => tpl.key);
}

// Diff tussen de gekozen verzorging-checklist en de al bestaande taken. `selection` is
// { [key]: { on, interval } } uit het aanpas-sheet. Levert:
//   - addKeys/overrides: nieuw-aangevinkt dat nog geen taak had → aanmaken (met interval)
//   - removeIds:         uitgevinkt dat er wél was → de gekoppelde taak-id's verwijderen
// (Het interval van een bestaande taak pas je via de taak zelf aan — niet hier.)
/**
 * @param {Array<{ key: string, title: string }>} templates
 * @param {Array<{ id?: any, title?: string | null }>} tasks
 * @param {Record<string, { on: boolean, interval: number }>} selection
 * @returns {{ addKeys: string[], overrides: Record<string, number>, removeIds: any[] }}
 */
export function diffCareSelection(templates, tasks, selection) {
  const present = new Set(presentCareKeys(templates, tasks));
  /** @type {string[]} */
  const addKeys = [];
  ...
  for (const tpl of templates) {
    const st = selection[tpl.key];
    if (!st) continue;
    const isPresent = present.has(tpl.key);
    if (st.on && !isPresent) {
      addKeys.push(tpl.key);
      overrides[tpl.key] = st.interval;
    } else if (!st.on && isPresent) {
      for (const task of tasks) if (isCareTaskFor(task, tpl)) removeIds.push(task.id);
    }
  }
  return { addKeys, overrides, removeIds };
}
