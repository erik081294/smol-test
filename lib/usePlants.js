...
// Dagboekfoto verwijderen: storage-object + rij weg. Was het de omslagfoto, dan
// valt de omslag terug op de eerstvolgende (nieuwste) resterende foto, of null.
// Geeft het nieuwe omslagpad terug.
export async function deletePlantPhoto({ photo, plant }) {
  await supabase.storage.from(PLANT_BUCKET).remove([photo.photo_path]).catch(() => {});
  await mutate(supabase.from('plant_photos').delete().eq('id', photo.id), { context: 'foto verwijderen' });

  if (plant?.photo_path === photo.photo_path) {
    const remaining = await run(
      supabase.from('plant_photos').select('photo_path')
        .eq('plant_id', photo.plant_id).order('created_at', { ascending: false }).limit(1),
