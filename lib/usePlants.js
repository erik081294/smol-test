...
export function usePlants() {
  ...
  return {
    plants: c.items, loading: c.loading, error: c.error, reload: c.reload,
...
export async function deletePlantPhoto({ photo, plant }) {
  return deleteDiaryEntryWithCover({
    entry: photo,
    parentCover: plant?.photo_path ?? null,
    removeObject: (path) => supabase.storage.from(PLANT_BUCKET).remove([path]).catch(() => {}),
    deleteRow: (id) => mutate(supabase.from('plant_photos').delete().eq('id', id), { context: 'tijdlijnpost verwijderen' }),
    fetchRemaining: () => run(
    ...
    setCover: (next) => mutate(supabase.from('plants').update({ photo_path: next }).eq('id', photo.plant_id), { context: 'omslag bijwerken' }),
  });
}
