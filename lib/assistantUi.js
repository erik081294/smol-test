...
// plan 26: interactieve gen-UI — grafiek, rooster, beslis-kaart) + image/progress
// en de lijn-variant op chart (AI-16 ronde 3, plan 28 sessie 6). Uitbreiden =
// expliciete afweging (§9, max +3 per ronde), geen gewoonte.

export const CATALOG_TYPES = ['text', 'card', 'list', 'keyvalue', 'confirm_action', 'link', 'recipe', 'chart', 'schedule', 'choice', 'image', 'progress'];

// De enige buckets waaruit een image-node mag tonen (AI-16 ronde 3): onze eigen
// private entity-buckets. De client signt het pad zelf via de RLS-gescopete
// storage-API (eerste pad-segment = household_id) — een pad buiten je huishouden
// levert dus nooit een URL op. De whitelist voorkomt dat model-/tool-output een
// vreemde bucket of een publieke URL de renderer in praat.
export const IMAGE_BUCKETS = ['plants', 'pets', 'recipes', 'vehicles', 'timeline'];
  ...
  switch (node.type) {
      ...
      const ingredients = Array.isArray(node.ingredients)
        ? node.ingredients
            .map((it) => {
              ...
              // Number.isFinite is type-strikt (geen coercion), dus één check
              // dekt "is een getal" én "is eindig" — geen dubbele verdediging
              // die de mutatietest niet kan onderscheiden.
              const structured = isNonEmptyString(it.name) && Number.isFinite(it.quantity) && it.quantity > 0;
              return structured
                ? { text: it.text, name: it.name, quantity: it.quantity, unit: isNonEmptyString(it.unit) ? it.unit : null }
                : { text: it.text };
            })
            .filter(Boolean)
        : [];
    ...
    case 'chart': {
      ...
      const points = Array.isArray(node.points)
        ? node.points
            .map((p) =>
              // Number.isFinite is type-strikt: één check voor "getal én eindig".
              isNonEmptyString(p?.label) && Number.isFinite(p?.value) && p.value >= 0
      ...
      return {
        ...
        // Ronde 3: lijn-variant voor trends (maanden); al het andere blijft staaf.
        variant: node.variant === 'line' ? 'line' : 'bar',
        points,
      };
    }
    case 'schedule': {
    ...
    case 'image': {
      // Foto uit eigen storage (AI-16 ronde 3). Alleen een kaal opslagpad uit
      // een gewhiteliste bucket — nooit een URL (extern laden is een exfil-/
      // tracking-kanaal). Pad-traversal en absolute paden vervallen; het
      // signen (en dus de autorisatie) doet de client zelf via storage-RLS.
      if (!IMAGE_BUCKETS.includes(node.bucket)) return null;
      if (!isNonEmptyString(node.path)) return null;
      ...
      return {
        ...
        bucket: node.bucket,
        path,
        caption: isNonEmptyString(node.caption) ? node.caption : null,
      };
    }
    case 'progress': {
      // Voortgangsbalk (AI-16 ronde 3): label + teller/noemer. Zonder zinnige
      // noemer (> 0) is er geen fractie te tekenen → droppen; de waarde klemt
      // op [0, max] zodat de balk nooit uit zijn spoor loopt.
      if (!isNonEmptyString(node.label)) return null;
      if (!Number.isFinite(node.value) || node.value < 0) return null;
      if (!Number.isFinite(node.max) || node.max <= 0) return null;
      return { type: 'progress', label: node.label, value: Math.min(node.value, node.max), max: node.max };
    }
    default:
...
export function treeToText(nodes = []) {
  ...
  for (const n of nodes) {
    if (n.type === 'text') parts.push(n.text);
      ...
      parts.push(n.caption ?? '(foto)');
    } else if (n.type === 'progress') {
      parts.push(`${n.label}: ${n.value} van ${n.max}`);
    }
  }
  return parts.join('\n');
}
