...
// gebruiker over een AI-voorstel beslist). Uitbreiden = expliciete afweging (§9,
// max +3 per ronde), geen gewoonte.

export const CATALOG_TYPES = ['text', 'card', 'list', 'keyvalue', 'confirm_action', 'link', 'recipe'];
  ...
  switch (node.type) {
    ...
    case 'recipe': {
      ...
      const ingredients = Array.isArray(node.ingredients)
        ? node.ingredients
            .map((it) => (isNonEmptyString(it?.text) ? { text: it.text } : null))
            .filter(Boolean)
        : [];
