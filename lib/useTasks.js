...
export function useTasks() {
  ...
  const completeTask = async (task) => {
    ...
    if (task.recur_freq && task.due_date) {
      // Doorrollen — tenzij de reeks is afgelopen (einddatum/aantal keren bereikt),
      // dan valt 'ie door naar een gewone voltooiing hieronder (UX, batch 2).
      const { next, count } = advanceRecurrence(task);
      if (next) {
        patch = { due_date: format(next, 'yyyy-MM-dd'), completed_at: null, completed_by: null };
        if (count != null) patch.recur_count = count;   // "na X keer" telt af
