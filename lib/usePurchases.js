...
// Query-venster (PERF-8): laad alleen de N nieuwste bonnen i.p.v. de hele historie.
// Bonnen groeien onbeperkt; de prijstracker/aankoopfrequentie lopen via eigen,
// product-gescopete queries (useProductPrices/useProductFrequencies), dus de volledige
// lijst is hier niet nodig.
const PURCHASES_WINDOW = 200;

export function usePurchases() {
  const { activeId } = useHousehold();
  const { user } = useAuth();

  // Stale-while-revalidate + huishouden-wissel-reset via de gedeelde hook (PERF-2).
  const key = activeId ? cacheKey('purchases', activeId) : null;
  const [purchases, setPurchases, loading, setLoading] = useCachedCollection(key);
  // Laadfout blootgelegd zodat het scherm een banner + retry toont i.p.v. een lege lijst
  // die niet van "geen bonnen" te onderscheiden is (P0-review 2026-07-02).
  const [error, setError] = useState(null);

  const load = useCallback(async () => {
    if (!activeId) { setPurchases([]); setError(null); setLoading(false); return; }
    const { data, error: qErr } = await runResult(
      supabase
        ...
        .limit(PURCHASES_WINDOW),
      { context: 'bonnen laden' }
    );
    if (qErr) { setError(qErr); setLoading(false); return; }
    setError(null);
    const rows = data ?? [];
    setPurchases(rows);
    setCached(cacheKey('purchases', activeId), rows);
    setLoading(false);
  }, [activeId]);
  ...
  return { purchases: visible, loading, error, reload: load, addPurchase, updatePurchase, deletePurchase, user };
}
