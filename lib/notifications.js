...
import { parseISO, isSameDay, differenceInCalendarDays, addDays } from 'date-fns';
...
export function pantryAlerts(items = [], { now = new Date(), time = '08:00', soonDays = 2 } = {}) {
  ...
  // addDays schuift een kalenderdag en behoudt de wandklok-tijd over een DST-grens heen;
  // een vaste +86.400.000 ms zou rond de zomer-/wintertijdovergang een uur verspringen.
...
// Stabiele signatuur van een geplande set (P1): zodat de impure hook een dure
// cancelAll + herplanning kan overslaan als de bron-data van identiteit wisselde maar
// inhoudelijk hetzelfde blijft (elke realtime-reload geeft nieuwe array-referenties).
// De fireAt is een vaste kalendertijd (geen now+delta), dus stabiel over herberekeningen.
export function remindersSignature(reminders = []) {
  return reminders
    .map((r) => `${r.id}${r.fireAt instanceof Date ? r.fireAt.getTime() : +new Date(r.fireAt)}${r.title ?? ''}${r.body ?? ''}`)
    .join('');
}
