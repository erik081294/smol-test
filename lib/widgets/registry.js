    ...
    <View style={{ gap: 4 }}>
      {prog.total > 0 ? (
        <>
          ...
          <Mini>{t('widget.taken.progress', { done: prog.done, total: prog.total })}</Mini>
        </>
      ) : null}
      {s.items[0] ? <Mini>{s.items.slice(0, 2).map((it) => it.title).join(' · ')}</Mini> : null}
    </View>
  ) : null;
  return (
...
export const WIDGETS = [
  ...
  { key: 'agenda.upcoming', module: 'agenda', title: t('agenda.title'), icon: 'agenda', sizes: SIZES, defaultSize: '2x1', isDefault: true, Render: AgendaUpcoming },
