...
function MealsTonight({ size, style, details }) {
  ...
  // 7-dagen-strip: per dag een stip — gevuld = gepland, leeg = nog te plannen; vandaag krijgt
  // een rand. Benut de breedte van de tegel i.p.v. een tekstregel.
  const preview = (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
      {s.week.map((d) => (
        <View key={d.date} style={{ alignItems: 'center', flex: 1, gap: 3 }}>
          <Text style={[type.caption, { fontSize: 10, color: d.today ? colors.forest : colors.inkFaint, fontWeight: d.today ? '800' : '500' }]}>
            {format(parseISO(d.date), 'EEEEEE', { locale: nl })}
          </Text>
          <View style={{
            width: 22, height: 22, borderRadius: 11,
            backgroundColor: d.planned ? colors.forest : colors.surfaceAlt,
            borderWidth: d.today ? 2 : 0, borderColor: colors.forest,
            alignItems: 'center', justifyContent: 'center',
          }}>
            {d.planned ? <Icon name="check" size={12} color={colors.onDark} weight="bold" /> : null}
          </View>
        </View>
      ))}
    </View>
