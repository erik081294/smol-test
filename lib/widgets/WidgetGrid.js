...
import { packGrid, widgetShowsDetails } from './grid';
...
// Vinger-drag-herschikking alleen op native. Op (mobiel) web zet react-native-gesture-
// handler `touch-action: none` op het gesture-element zodat het pointer-events kan
// ontvangen; daardoor weigert de browser de pagina te scrollen zodra je vinger op een
// widget start (op native laat RNGH de aanraking wél door naar de ScrollView tot de
// long-press activeert). Web herschikt daarom via de toegankelijke controlebalk
// (renderControls: vooruit/terug-pijlen in bewerkmodus) i.p.v. slepen — scrollen en
// tikken blijven dan onaangetast. Eén gate, geldt automatisch voor elke widget.
...
export function WidgetGrid({
  ...
  scrollSV, onEdgeScroll, onEdgeScrollEnd,
}) {
  ...
  const detailsByKey = useMemo(
    () => Object.fromEntries(layout.map((p) => [p.key, widgetShowsDetails(p)])),
    [layout],
  );
      ...
      {order.map((key, index) => {
        ...
        return (
          <DragCell
            ...
            onEdgeScrollEnd={onEdgeScrollEnd}
          >
            <View pointerEvents={editing ? 'none' : 'auto'} style={{ height: tileH, overflow: 'hidden' }}>
              <descriptor.Render size={sizeByKey[key]} details={detailsByKey[key]} style={widgetStyle} tasks={tasks} members={members} />
            </View>
...
function DragCell({ cellKey, index, rect, cellH, editing, reduce, shared, scrollSV, onDragStart, onDragMove, onDragEnd, onEdgeScroll, onEdgeScrollEnd, children }) {
  const { activeKey, dragX, dragY, grabX, grabY } = shared;
  // Scroll-offset bij het oppakken — zodat de tegel de vinger blijft volgen terwijl de
  // pagina meescrollt tijdens auto-scroll (anders zou 'ie wegdrijven).
  const grabScroll = useSharedValue(0);
  ...
  const pan = useMemo(() => Gesture.Pan()
    ...
    .onStart(() => {
      ...
      grabScroll.value = scrollSV ? scrollSV.value : 0;
      dragX.value = 0;
      dragY.value = 0;
      runOnJS(onDragStart)();
    })
    .onUpdate((e) => {
      ...
      // Scroll-delta sinds oppakken: de hit-test rekent in content-coördinaten, dus tel
      // de meegescrollde afstand erbij op (anders mikt 'ie op de verkeerde cel na scroll).
      const sd = scrollSV ? scrollSV.value - grabScroll.value : 0;
      runOnJS(onDragMove)(cellKey, grabX.value + e.translationX + halfW, grabY.value + e.translationY + halfH + sd);
      if (onEdgeScroll) runOnJS(onEdgeScroll)(e.absoluteY);
    })
    .onEnd(() => {
    ...
    .onFinalize(() => {
      ...
      if (onEdgeScrollEnd) runOnJS(onEdgeScrollEnd)();
    }),
  ...
  const animStyle = useAnimatedStyle(() => {
    ...
    if (active) {
      return {
        ...
        // + scroll-delta: de tegel blijft onder de vinger terwijl de pagina meescrollt.
        top: grabY.value + dragY.value + (scrollSV ? scrollSV.value - grabScroll.value : 0),
        opacity: 1,
        zIndex: 999,
  ...
  // Op web: géén GestureDetector eromheen (zou via touch-action:none de browser-scroll
  // blokkeren). De cel rendert verder identiek — herschikken loopt dan via de pijlen in
  // de controlebalk; de layout-spring (animStyle) animeert die verplaatsing nog steeds.
  if (!DRAG_ENABLED) {
    return (
