...
// Of een (brede) widget zijn details/preview toont. Standaard aan (undefined ⇒ true),
// zodat bestaande layouts hun preview houden. In een 1×1-tegel is er geen ruimte voor
// een preview; daar heeft de vlag geen zichtbaar effect (de tegel toont alleen de stat).
export function widgetShowsDetails(placement) {
  return placement?.details !== false;
}

// Schakel de details/preview van een widget aan/uit (pure transform).
export function toggleWidgetDetails(layout, key) {
  return (layout ?? []).map((x) => (
    x.key === key ? { ...x, details: !widgetShowsDetails(x) } : x
  ));
}
