...
export function mealPlanSummary(entries, now = new Date()) {
  ...
  // Voor de 7-dagen-strip in de widget: per dag (ma–zo) of er iets gepland is + of het
  // vandaag is. De volgorde is de weekvolgorde (maandag eerst).
  const week = days.map((d) => ({ date: d, planned: planned.has(d), today: d === today }));
  // De gerechten die eraan komen (vanaf vandaag, alleen geplande dagen) — diner-entry of
  // de eerste van die dag. Voedt de "wat eten we"-regels in de widget.
  const entryForDay = (d) => {
    const de = (entries ?? []).filter((e) => e.plan_date === d);
    return de.find((e) => e.meal_type === 'diner') ?? de[0] ?? null;
  };
  const upcoming = days.filter((d) => d >= today && planned.has(d)).map((d) => ({ date: d, entry: entryForDay(d) }));
  return { tonight, emptyDays, emptyCount: emptyDays.length, week, upcoming };
}
