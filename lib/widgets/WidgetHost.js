...
export function WidgetTile({
  ...
  const iconChip = (dim) => (
    <View style={{
      ...
      <Icon name={icon} size={Math.round(dim * 0.52)} color={style === 'playful' ? colors.onDark : scheme.icon} />
    </View>
  );
  const heading = (
    <>
      ...
      {stat ? (
        <Text style={[type.title, { fontSize: 17, color: statColor ?? scheme.accent, fontWeight: '800', marginTop: 2 }]} numberOfLines={2}>
          {stat}
        </Text>
      ) : null}
    </>
      ...
      {listLayout ? (
        ...
        <View style={{ flex: 1 }}>
          <Row align="center" gap={space.sm} style={{ marginBottom: space.xs }}>
            ...
            {stat ? (
              <Text style={[type.caption, { color: statColor ?? scheme.accent, fontWeight: '700', maxWidth: '46%' }]} numberOfLines={1}>
                {stat}
              </Text>
            ) : null}
