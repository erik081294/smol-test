    ...
    >
      <Row justify="space-between" align="center">
        <View style={{
          ...
          backgroundColor: style === 'playful' ? colors.surface : colors.surfaceAlt,
          alignItems: 'center', justifyContent: 'center',
        }}>
          <Icon name={icon} size={20} color={scheme.icon} />
        </View>
      ...
      {stat ? (
        <Text style={[type.body, { color: statColor ?? scheme.accent, fontWeight: '800', marginTop: 2 }]} numberOfLines={1}>
          {stat}
        </Text>
      ) : null}
      {wide && preview ? <View style={{ marginTop: space.xs }}>{preview}</View> : null}
      {wide && children ? <View style={{ marginTop: space.xs }}>{children}</View> : null}
    </Pressable>
  );
}
