...
// Schoonmaak (SCH-4): `cleaningOnly` houdt alleen zone-gebonden taken over (het
// "rooster"); `zoneId` knijpt verder in op één zone. Een schoonmaaktaak is per
// definitie een taak met een zone_id (zie schoonmaak.js), dus dit is de zone-as
// waarmee de Taken-tab een schoonmaakrooster per week/maand kan tonen.
export function applyTaskFilters(tasks, { categories = [], assignees = [], subgroupId = null, tagIds = [], status = 'open', audience = 'all', viewerId = null, cleaningOnly = false, zoneId = null } = {}) {
  ...
  return (tasks ?? []).filter((t) => {
    ...
    if (cleaningOnly && t.zone_id == null) return false;
    if (zoneId && t.zone_id !== zoneId) return false;
...
export function activeFilterCount({ categories = [], assignees = [], subgroupId = null, tagIds = [], status = 'open', cleaningOnly = false, zoneId = null } = {}) {
  let n = 0;
