...
import { colors, categoryMeta, font } from './theme';
...
// Gememoïseerd (PERF-4): bij een afvink hertekent alleen de rij waarvan de `task`
// (of een andere prop) wijzigt — niet de hele lijst. Werkt overal waar TaskRow stabiele
// props krijgt (taken/agenda/vandaag). Stabiliseer aan de aanroepkant `members`/`tags`
// en de callbacks, anders valt het voordeel weg.
export const TaskRow = React.memo(function TaskRow({ task, members, tags, onToggle, onPress }) {
  ...
  const meta = (
    <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 5 }}>
      ...
      <Text style={{ fontSize: 12, color: colors.inkSoft, fontFamily: font.semi }}>{cat.label}</Text>
      {task.zone ? <Dot /> : null}
      {task.zone ? (
