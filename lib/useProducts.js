...
import { uploadPhoto, deletePhotoObjects } from './photoStorage';
import { normalizeExt } from './plantPhoto';

// Private bucket voor producteditor-foto's (migr. 0062, household-gescopete RLS).
...
export function useProducts() {
  ...
  // Zet/vervang de foto van een product (BOO-13). Uniek pad per upload
  // (<household_id>/<product_id>/<key>.<ext>) zodat de signed-URL-cache vanzelf bust en
  // er geen stale beeld blijft hangen; de vorige foto ruimen we best-effort op. Het
  // eerste pad-segment is het household_id → de storage-RLS (0062) scopet erop. Geldt
  // huishouden-breed (schrijft photo_path op de gedeelde rij).
  const setProductPhoto = async (id, asset) => {
    if (!asset?.base64) return null;
    const prev = c.items.find((p) => p.id === id);
    const key = Date.now().toString(36);
    const path = `${c.activeId}/${id}/${key}.${normalizeExt(asset.ext)}`;
    await uploadPhoto({ bucket: PRODUCT_BUCKET, path, base64: asset.base64, ext: asset.ext });
    await c.update(id, { photo_path: path });
    if (prev?.photo_path) deletePhotoObjects(PRODUCT_BUCKET, [prev.photo_path]);
    return path;
  };

  // Verwijder de foto van een product (best-effort opruimen van het opslag-object).
  const clearProductPhoto = async (id) => {
    const prev = c.items.find((p) => p.id === id);
    await c.update(id, { photo_path: null });
    if (prev?.photo_path) deletePhotoObjects(PRODUCT_BUCKET, [prev.photo_path]);
  };
...
export function useProductFrequencies() {
  ...
  const load = useCallback(async () => {
    ...
    // Server-side aggregatie (PERF-8): de RPC groepeert de bonregels van de laatste 12
    // maanden per product in de DB (benut de index uit migr. 0045) en geeft één rij per
    // product met z'n aankoopdatums terug — i.p.v. álle bonregels client-side op te halen
    // en te groeperen. De pure heuristiek (frequencyEstimate) blijft de bron van waarheid.
    const rows = await run(
      supabase.rpc('product_purchase_dates', { p_household: activeId, p_months: 12 }),
      { fallback: [], context: 'aankoopfrequentie laden' }
    );
    const out = {};
    for (const r of rows ?? []) {
      const est = frequencyEstimate(r.dates ?? []);
      if (r.product_id && est) out[r.product_id] = est;
    }
    setByProduct(out);
  }, [activeId]);
