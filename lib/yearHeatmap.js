 ...
 * @param {{ weekStartsOn?: 0|1|2|3|4|5|6, today?: Date }} [opts]
 */
export function yearGrid(year, counts = new Map(), { weekStartsOn = 1, today = new Date() } = {}) {
  ...
  for (let i = 0; i < totalDays; i += 1) {
    ...
    const key = /** @type {string} */ (localDayKey(date));
...
// De beste losse dag tonen we niet apart: dat is de donkerste cel in het raster en
// komt al naar voren door op een dag te tikken. `today` injecteerbaar voor tests.
export function yearSummary(counts = new Map(), year, { today = new Date() } = {}) {
  ...
  const todayKey = localDayKey(today);
  const startKey = /** @type {string} */ (localDayKey(yearStart));
  const endKey = /** @type {string} */ (localDayKey(yearEnd));
  let currentStreak = 0;
  if (todayKey != null && todayKey >= startKey) {
    let cursor = todayKey <= endKey
      ? new Date(today.getFullYear(), today.getMonth(), today.getDate())
      : new Date(year, 11, 31);
    while (/** @type {string} */ (localDayKey(cursor)) >= startKey && dayCount(cursor) > 0) {
      currentStreak += 1;
  ...
  return { total, activeDays, longestStreak, currentStreak, busiestWeekday, byWeekday };
}

// De rasterkolom (week-index) die de "vandaag"-cel bevat, of -1 als die er niet is
// (een afgelopen of toekomstig jaar heeft geen `isToday`). Voedt het auto-scrollen
// van de horizontale heatmap (lib/YearHeatmapView.js): bij het lopende jaar willen we
// vandaag in beeld i.p.v. helemaal links op 1 januari.
/**
 * @param {{ weeks?: { isToday?: boolean }[][] }} [grid]
 * @returns {number}
 */
export function todayColumn(grid) {
  const weeks = grid?.weeks ?? [];
  for (let col = 0; col < weeks.length; col += 1) {
    if (weeks[col].some((cell) => cell.isToday)) return col;
  }
  return -1;
}

// Horizontale scroll-offset (px) zodat de `col`-kolom rechts in beeld komt — met wat
// lucht erachter — i.p.v. dat het raster op 1 januari blijft staan. Puur reken-werk
// zodat de view dun blijft en dit getest is. Geeft 0 als er niets te scrollen valt
// (geen today-kolom, of de viewport is nog niet gemeten). Het resultaat is geklemd op
// het geldige bereik [0, contentBreedte − viewport]: nooit voorbij de randen.
/**
 * @param {{
 *   col: number, viewportWidth: number, gridWidth: number,
 *   colWidth: number, labelWidth?: number, padRight?: number,
 * }} p
 * @returns {number}
 */
export function heatmapScrollX({
  col, viewportWidth, gridWidth, colWidth, labelWidth = 0, padRight = 0,
}) {
  if (col < 0 || viewportWidth <= 0) return 0;
  const contentWidth = labelWidth + gridWidth + padRight;
  const maxScroll = Math.max(0, contentWidth - viewportWidth);
  // Rechterrand van de today-kolom (incl. de label-kolom die meeschuift) + lucht,
  // daarna links uitlijnen op de viewport-breedte.
  const desired = labelWidth + (col + 1) * colWidth + padRight - viewportWidth;
