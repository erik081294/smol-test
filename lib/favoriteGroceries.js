...
// Catalogus-zoek over de EIGEN huishoud-producten (BOO-13): zo vind je een zelf-aangemaakt
// product terug op naam, niet alleen de gebundelde catalogus. Anders dan `recentProducts`
// telt `last_added_at` hier NIET mee — ook een product dat nog nooit op de lijst stond moet
// vindbaar zijn, anders zoek je 'm, krijgt "Niets gevonden" en maak je per ongeluk een
// duplicaat. Sorteert op gebruik → recentheid → naam (`byUsage`). Lege query → niets.
export function searchOwnProducts(products = [], { query = '', n = 24 } = {}) {
  const q = normalize(query || '');
  if (!q) return [];
  return products
