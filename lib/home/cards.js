...
// Activiteit — de laatste paar gebeurtenissen in het huishouden (PLT-6). Niets
// gebeurd → geen kaart. Toont de nieuwste als stat + maximaal 2 erna als preview
// ("laatste 3"), en linkt door naar het volledige Activiteit-scherm.
function ActivityCard() {
  ...
  return (
    <SummaryCard
      icon="feed"
      title={t('home.card.activity.title')}
      tone="neutral"
      stat={first.text}
      preview={more.length > 0
        ? <View>{more.map((it) => <Text key={it.id} style={type.caption} numberOfLines={1}>{it.text}</Text>)}</View>
        : <Preview>{first.when}</Preview>}
