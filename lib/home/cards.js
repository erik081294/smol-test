...
// Maaltijden — wat eten we vandaag (deze week-hook, gefilterd op vandaag).
function MealsCard() {
  const router = useRouter();
  ...
  return (
    <SummaryCard
      ...
      title={t('meals.title')}
      tone="neutral"
      stat={empty ? t('home.card.meals.none') : (diner.recipe?.title || diner.title || t('mealtype.' + diner.meal_type))}
      preview={empty ? null : <Preview>{t('recipe.tonight')}</Preview>}
      onPress={() => router.push('/(tabs)/maaltijden')}
    />
  );
}

// Voorraad — hoeveel producten bijna op / over datum zijn.
function PantryCard() {
  const router = useRouter();
  ...
  const empty = urgent.length === 0;
  return (
    <SummaryCard
      icon="pantry"
      title={t('pantry.title')}
      tone={empty ? 'positive' : 'urgent'}
      stat={empty ? t('home.card.pantry.ok') : plural(urgent.length, 'home.card.pantry.one', 'home.card.pantry.other')}
      preview={empty ? null : <Preview>{urgent.slice(0, 3).map((p) => p.name).join(' · ')}{urgent.length > 3 ? ' …' : ''}</Preview>}
