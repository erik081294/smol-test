 ...
 *
 * @param {Array<{completed_by?: string|null, completed_at?: string|null}>} [completions] uit task_completions
 * @param {Array<{id: string, display_name?: string, avatar_emoji?: string}>} [members]
 * @param {Date|null} [since] null = alle tijd; oudere voltooiingen tellen niet
 */
export function tally(completions = [], members = [], since = null) {
  ...
  for (const c of completions) {
    ...
    // has()-guard hierboven garandeert dat de key bestaat; de cast onderdrukt enkel
    // TS' beperking dat Map.get niet via Map.has narrowt (geen runtime-effect).
    counts.set(c.completed_by, /** @type {number} */ (counts.get(c.completed_by)) + 1);
  }
