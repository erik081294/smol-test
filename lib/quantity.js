...
// Strikte numerieke parser voor een typbaar hoeveelheid-veld (recept-ingrediënt, MLT).
// Anders dan `parseQuantity` (die een naam/eenheid mee-leest en altijd op 1 terugvalt)
// geeft deze `null` zodra de invoer géén schoon positief getal is — zo kan de UI een
// half-getypte/lege waarde negeren i.p.v. er een 1 van te maken. Komma én punt tellen
// als decimaalteken (NL), omliggende spaties mogen.
//   '250' -> 250 · '1,5' -> 1.5 · '0.5' -> 0.5 · '' / 'abc' / '-2' / '0' -> null
export function parseAmount(text) {
  const s = String(text ?? '').trim().replace(',', '.');
  if (!/^\d+(?:\.\d+)?$/.test(s)) return null;
  const v = round3(parseFloat(s));
  return v > 0 ? v : null;
}
