...
// Een datum-only ISO-string ('2026-06-01') is een kalenderdag, geen instant: new Date()
// parst 'm als UTC-middernacht, wat in negatieve-offset-tijdzones een dag terugschuift.
// Daarom interpreteren we 'm expliciet als lokale dag. Volledige timestamps (met tijd/
// zone) zijn ondubbelzinnig en worden lokaal afgelezen, zoals bedoeld.
...
// Sorteer aankoopdatums tot unieke dag-timestamps (oplopend). Dedup op kalenderdag:
// twee aankopen op dezelfde dag (dayMs → identieke middernacht-ms) zouden anders een
// interval van 0 introduceren en de mediaan naar beneden trekken (halveren).
function sortedDays(dates) {
  const ms = [...(dates ?? [])].map(dayMs).filter((x) => x != null);
...
export function frequencyEstimate(dates, now = new Date()) {
  ...
  const today = /** @type {number} */ (dayMs(now));
  const daysSince = Math.round((today - last) / DAY);
  // medianDays is hier gegarandeerd >= 1: sortedDays dedupliceert op kalenderdag, dus elk
  // interval is >= 1 dag en de (afgeronde) mediaan dus >= 1 — een deling door 0 kan niet
  // optreden. (Vroeger kon een same-day-interval van 0 de mediaan op 0 brengen; die tak is
  // met de dedup dode code geworden, daarom weg i.p.v. een onbereikbare guard.)
  const dueScore = daysSince / medianDays;
