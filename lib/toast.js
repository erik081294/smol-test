...
import { colors, space, radius, type, elevation, font } from './theme';

// Hoogte waarop de toast zweeft. Boven de tabbalk (86) op tab-schermen; op
// modal-editors zweeft 'ie wat hoger — bewust, blijft leesbaar. Onder edge-to-edge
// groeit de tabbalk met de systeem-navigatie-inset mee, dus tellen we die onder-
// inset bij de offset op zodat de toast er bóven blijft (backlog UX-5).
...
export function ToastProvider({ children }) {
  ...
  return (
    <ToastContext.Provider value={{ show }}>
      ...
      {toast ? (
        <View
          // Live-region (A11Y-1): VoiceOver/TalkBack leest de toast-boodschap voor zodra
          // 'ie verschijnt — anders mist een screenreader-gebruiker o.a. de "ongedaan
          // maken"-melding na een vernietigende actie. De actieknop blijft apart focusbaar.
          accessibilityLiveRegion="polite"
          accessibilityRole="alert"
          style={{ pointerEvents: 'box-none', position: 'absolute', left: 0, right: 0, bottom: BOTTOM_OFFSET + insets.bottom, alignItems: 'center', paddingHorizontal: space.lg }}>
          <View style={[{
            flexDirection: 'row', alignItems: 'center', gap: space.md,
            backgroundColor: colors.toastBg, borderRadius: radius.md,
            paddingVertical: space.md, paddingHorizontal: space.lg, maxWidth: 520, width: '100%',
          }, elevation.e3]}>
            <Text style={[type.body, { color: colors.toastText, flex: 1 }]}>{toast.message}</Text>
            {toast.actionLabel ? (
              <Pressable onPress={onAction} hitSlop={10} accessibilityRole="button" accessibilityLabel={toast.actionLabel}>
                <Text style={{ color: colors.ocher, fontFamily: font.semi, fontSize: 15 }}>{toast.actionLabel}</Text>
              </Pressable>
            ) : null}
          </View>
        </View>
