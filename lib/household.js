...
export function HouseholdProvider({ children }) {
  ...
  const createSubgroup = async (name, emoji, memberIds) => {
    ...
    if (rows.length) await supabase.from('subgroup_members').insert(rows);
    await loadSubgroups();
    return data;
  };
  ...
  const updateSubgroupMembers = async (subgroupId, memberIds) => {
    await supabase.from('subgroup_members').delete().eq('subgroup_id', subgroupId);
  ...
  const deleteSubgroup = async (subgroupId) => {
    await supabase.from('subgroups').delete().eq('id', subgroupId);
    await loadSubgroups();
  };
