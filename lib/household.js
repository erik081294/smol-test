...
// Puur en buiten de component zodat de useCallback's hieronder er niet op hoeven te deppen.
...
export function HouseholdProvider({ children }) {
  ...
  //
  // Alle handlers hieronder zijn met useCallback gestabiliseerd (identiteits-stabilisatie,
  // review 2026-07-02 Perf-medium): samen met de value-useMemo onderaan her-rendert een
  // consumer alleen nog als er écht iets wijzigt, niet bij elke Provider-render.
  const setHouseholdModule = useCallback(async (key, enabled) => {
  ...
  }, [user, activeId, loadModuleSettings]);
  ...
  const createSubgroup = useCallback(async (name, emoji, memberIds) => {
    ...
    await loadSubgroups();
    return data;
  }, [activeId, user, loadSubgroups]);

  // Subgroep-leden vervangen
  const updateSubgroupMembers = useCallback(async (subgroupId, memberIds) => {
    await mutate(
  ...
  const deleteSubgroup = useCallback(async (subgroupId) => {
    await mutate(
  ...
  const selectHousehold = useCallback(async (id) => {
    setActiveId(id);
  ...
  const createHousehold = useCallback(async (name, emoji) => {
    ...
    return data;
  }, [loadHouseholds, selectHousehold]);
  ...
  const createInvite = useCallback(async (role = 'member') => {
  ...
  const acceptInvite = useCallback(async (token) => {
    ...
    return data;
  }, [loadHouseholds, selectHousehold, clearPendingInvite]);
  ...
  const peekInvite = useCallback(async (token) => {
    const { data, error } = await supabase.rpc('peek_invite', { p_token: token });
    if (error) throw error;
    return Array.isArray(data) ? (data[0] ?? null) : (data ?? null);
  }, []);

  const leaveHousehold = useCallback(async (id) => {
    await mutate(
      supabase
        .from('household_members')
        .delete()
  ...
  // Context-value memoiseren (review 2026-07-02 Perf-medium): zonder deze memo is de
  // value elke Provider-render een níéuw object en her-rendert elke useHousehold-consumer
  // app-breed, ook als er niets wijzigde. Alle functies hierboven zijn useCallback-stabiel,
  // dus de value verspringt alleen nog als een veld écht verandert.
  const value = useMemo(() => ({
    ...
    selectHousehold, createHousehold, leaveHousehold,
    createSubgroup, updateSubgroupMembers, deleteSubgroup,
    invites, createInvite, revokeInvite, acceptInvite, peekInvite, loadInvites,
    pendingInvite, setPendingInvite, clearPendingInvite,
    loadHouseholds, loadMembers, loadSubgroups, loadModuleSettings,
  ]);

  return (
    <HouseholdCtx.Provider value={value}>
      {children}
