...
export function useResources() {
  ...
  return {
    resources: c.items, loading: c.loading, error: c.error, reload: c.reload,
...
export function useReservations(resourceId) {
  ...
  // Laden + realtime: reserveringen van deze ene resource.
  useRealtimeReload(load, resourceId, [
    { table: 'reservations', filter: `resource_id=eq.${resourceId}` },
  ], { name: 'reservations' });
