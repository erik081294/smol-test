...
function Node({ node, onAction, onEdit, onChoice }) {
  switch (node.type) {
    case 'text':
...
export function AssistantMessageView({ tree = [], onAction, onEdit, onChoice }) {
  if (tree.length === 0) return null;
  const bulkIds = onAction ? pendingActionIds(tree) : [];
  return (
    <Stack gap={space.sm}>
      {tree.map((node, i) => <View key={i}><Node node={node} onAction={onAction} onEdit={onEdit} onChoice={onChoice} /></View>)}
