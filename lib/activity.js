...
const FORMATTERS = {
  ...
  // "X voegde <iets> toe"-events (plant/huisdier/voertuig): zelfde vorm, via een fabriek.
  plant_added: addedFormatter('plant', 'plants'),
  pet_added: addedFormatter('huisdier', 'pets'),
  vehicle_added: addedFormatter('voertuig', 'voertuig'),
};

// Fabriek voor de uniforme "X voegde <noun> '<subject>' toe"-regel.
function addedFormatter(noun, icon) {
  return (e, count = 1) => {
    const actor = e.actorName?.trim() || 'Iemand';
    const subject = subjectOf(e)?.trim();
    if (!subject) return null;
    const times = count > 1 ? ` ${count}×` : '';
    return { icon, text: `${actor} voegde ${noun} ${q(subject)} toe${times}` };
  };
}
...
//   { id, type, at, actorName, actorId?, ...type-specifiek }
...
export function formatActivity(event, now = Date.now(), count = 1) {
  ...
  if (!base) return null;
  // `type` reist mee naar de feed: de tijdlijn-filter (TML-6, lib/timelineFilter.js)
  // beslist per event-type/module wat zichtbaar is — de UI-interne id-prefix is
  // daarvoor bewust niet betrouwbaar (die mag wijzigen). `actorId` (TML-7) reist om
  // dezelfde reden mee: de member-as filtert op profiel-id, niet op weergavenaam.
  const item = { id: event.id, type: event.type, at: event.at, actorId: event.actorId ?? null, when: relativeTime(event.at, now), ...base };
