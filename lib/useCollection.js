...
import { patchItem, removeItem, removeItems } from './optimisticList';
...
export function useCollection(table, {
  ...
  // De lijst-transformaties zelf zijn puur (lib/optimisticList.js, ratchet-bewaakt);
  // hier blijft alleen de React-kant: prev-capture + rollback bij een serverfout.
  const update = (id, patch) => {
    let prev;
    setItems((cur) => {
      prev = cur;
      return patchItem(cur, id, patch);
    });
    return mutate(
