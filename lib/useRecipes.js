...
export function useRecipes() {
  ...
  const addRecipe = ({ title, servings = 2, instructions = null, sourceUrl = null }) =>
    mutate(
...
export function useRecipe(recipeId) {
  ...
  useEffect(() => {
    if (!recipeId || recipeId === 'new') return undefined;
    const ch = supabase
      .channel(`recipe:${recipeId}:${Math.random().toString(36).slice(2)}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'recipe_ingredients', filter: `recipe_id=eq.${recipeId}` }, () => loadRef.current())
      .subscribe();
