...
export function useTimeline() {
  ...
  const load = useCallback(async () => {
    if (!activeId) { setPosts([]); setLoading(false); setHasMore(false); return; }
  ...
  const setPinned = (id, pinned) =>
    mutate(
      supabase.from('timeline_posts').update({ pinned_at: pinned ? new Date().toISOString() : null }).eq('id', id),
  ...
  return { posts: visiblePosts, loading, reload: load, loadMore, hasMore, addPost, setPinned, deletePost, members, user };
}
