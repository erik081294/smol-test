...
export function useMealPlan(weekStart = new Date()) {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel(`mealplan:${activeId}:${Math.random().toString(36).slice(2)}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'meal_plan_entries', filter: `household_id=eq.${activeId}` }, () => loadRef.current())
      .subscribe();
