...
const { subscribe: subscribeShared, teardownAll: teardownSharedHub } = createRealtimeHub(supabase);

// Forceer-opruimen van alle gedeelde realtime-kanalen. Aangeroepen bij logout (auth.js)
// zodat events van het vorige account niet kunnen blijven binnenkomen op één toestel.
export function teardownRealtime() {
  teardownSharedHub();
}

// Een burst realtime-events (bv. de bulk task-insert bij het aanmaken van een
// huisdier/plant/voertuig — meerdere care-taken ineens) zou per event een volle,
// vaak zware (join-)herlaad afvuren. We wachten dit korte venster ná het laatste
// event af en herladen dan één keer. Lang genoeg om een bulk-insert te bundelen,
// kort genoeg om responsief te blijven.
...
export function useRealtimeReload(loadFn, subscribeKey, sources, { name = 'rt', onChange } = {}) {
  ...
  // Debounce-timer voor het reload-pad (zie RELOAD_DEBOUNCE_MS). Het incrementele
  // onChange-pad wordt NIET gedebounced: daar moet elk event afzonderlijk gepatcht.
  ...
  useEffect(() => {
    ...
    // anders een (gedebouncede) full reload. Wordt door de hub aangeroepen per event.
    const cb = (payload) => {
      if (onChangeRef.current) { onChangeRef.current(payload); return; }
    ...
    const unsub = subscribeShared(subscribeKey, listeners, name);
    return () => {
