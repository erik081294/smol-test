...
import { diaryPhotoPath, collectPhotoPaths } from './plantPhoto';
import { uploadPhoto, signedUrl, useSignedUrl, deletePhotoObjects } from './photoStorage';
...
export function useVehicles() {
  ...
  // Voertuig verwijderen: ruim ook de boekje-foto's in storage op. De vehicle_log-rijen
  // cascaden in de DB, maar de storage-objecten niet — die zoeken we eerst op (vóór de
  // cascade ze weghaalt), verwijderen dan het voertuig en ruimen daarna best-effort op.
  const removeVehicle = async (id) => {
    const logs = await run(
      supabase.from('vehicle_log').select('photo_path').eq('vehicle_id', id).not('photo_path', 'is', null),
      { fallback: [], context: 'boekjefoto-paden opzoeken' }
    );
    const result = await c.remove(id);
    await deletePhotoObjects(VEHICLE_BUCKET, collectPhotoPaths(logs));
    return result;
  };

  return {
    vehicles: c.items, loading: c.loading, error: c.error, reload: c.reload,
    addVehicle, updateVehicle: c.update, removeVehicle, setVehicleShared,
  };
}
