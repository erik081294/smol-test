...
import { diaryPhotoPath, contentTypeForExt } from './plantPhoto';
...
// Signed URL voor een private boekje-foto (default 1 uur).
export async function signedVehiclePhotoUrl(path, expiresIn = 3600) {
  if (!path) return null;
  const { data } = await supabase.storage.from(VEHICLE_BUCKET).createSignedUrl(path, expiresIn);
  return data?.signedUrl ?? null;
}

// Hook: opslagpad → toonbare (signed) URL; null zolang er geen foto is.
export function useVehiclePhotoUrl(path) {
  const [url, setUrl] = useState(null);
  useEffect(() => {
    let on = true;
    if (!path) { setUrl(null); return undefined; }
    signedVehiclePhotoUrl(path).then((u) => { if (on) setUrl(u); });
    return () => { on = false; };
  }, [path]);
  return url;
}
...
export async function addVehicleLog({
  ...
  if (photoAsset?.base64) {
    ...
    photoPath = diaryPhotoPath(householdId, vehicle.id, key, photoAsset.ext);
    const { error } = await supabase.storage.from(VEHICLE_BUCKET).upload(
      photoPath, decode(photoAsset.base64),
      { contentType: contentTypeForExt(photoAsset.ext), upsert: true },
    );
