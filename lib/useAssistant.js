...
import { buildResolveBody, actionStatusMap, stampActionStatus } from './assistantActions';
...
export function useAssistant() {
  ...
  const resolveAction = useCallback(async (actionId, decision, selected) => {
    const body = buildResolveBody(actionId, decision, selected);
    if (!body) return;
    try {
  ...
  resolveRef.current = resolveAction;
