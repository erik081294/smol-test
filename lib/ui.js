  ...
  if (onConfirm) {
    ...
    return (
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: space.lg, paddingVertical: space.md }}>
        <Pressable onPress={onClose} testID="t-cancel" hitSlop={10} accessibilityRole="button" accessibilityLabel={cancelLabel}>
          <Text style={[type.title, { fontWeight: '600', color: colors.inkSoft }]}>{cancelLabel}</Text>
        </Pressable>
        <Text style={type.title}>{title}</Text>
        <Pressable onPress={onConfirm} disabled={inert} testID="t-save" hitSlop={10} accessibilityRole="button" accessibilityLabel={confirmLabel} accessibilityState={{ busy: !!busy, disabled: inert }}>
