...
import { Text, TextInput, Pressable, View, ActivityIndicator, ScrollView, Animated, Platform, KeyboardAvoidingView, Alert, BackHandler } from 'react-native';
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [{
        ...
        paddingHorizontal: extended ? space.xl : 0,
        flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: space.sm,
        backgroundColor: pressed ? '#CE9531' : colors.ocher,
      }, elevation.e3, style]}
    >
      <Icon name={icon} size={extended ? 22 : 28} color={colors.forest} weight="bold" />
...
export function Badge({ label, tone = 'neutral' }) {
  const tones = {
    ...
    brand:   { bg: colors.forestTint,   fg: colors.forest },
  }[tone] ?? {};
  return (
...
// ===========================================================================
export function ModalHeader({ title, onClose, onConfirm, confirmLabel = 'Bewaar', cancelLabel = 'Annuleer', busy }) {
  if (onConfirm) {
    return (
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: space.md }}>
      <Text style={[type.h1, { flex: 1 }]}>{title}</Text>
      <IconButton icon="close" accessibilityLabel="Sluiten" tint={colors.inkSoft} onPress={onClose} />
    </View>
  );
}
...
// ===========================================================================
// Platform-bewuste bevestiging bij het weggooien van niet-bewaarde wijzigingen.
// Alert op native; window.confirm op web (Alert-knoppen vuren daar niet — zie de
// undo-aanpak elders). Bevestigt de gebruiker, dan pas sluiten we echt.
function confirmDiscard(onConfirm) {
  if (Platform.OS === 'web') {
    if (typeof window !== 'undefined' && window.confirm(t('common.discard.body'))) onConfirm?.();
    return;
  }
  Alert.alert(t('common.discard.title'), t('common.discard.body'), [
    { text: t('common.discard.stay'), style: 'cancel' },
    { text: t('common.discard.confirm'), style: 'destructive', onPress: () => onConfirm?.() },
  ]);
}
  ...
  return (
    <View style={{ alignItems: 'center', paddingVertical: space.xxxl, paddingHorizontal: space.xl, gap: space.xs }}>
      ...
      {actionTitle && onAction
        ? <Button title={actionTitle} variant="accent" fullWidth={false} onPress={onAction} style={{ marginTop: space.md }} />
        : null}
    </View>
  );
}
