  ...
  if (onConfirm) {
    ...
    return (
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: space.lg, paddingVertical: space.md }}>
        ...
        <Pressable onPress={onConfirm} disabled={inert} hitSlop={10} accessibilityRole="button" accessibilityLabel={confirmLabel} accessibilityState={{ busy: !!busy, disabled: inert }}>
          <Text style={[type.title, { fontWeight: '800', color: colors.forest, opacity: inert ? 0.5 : 1 }]}>{confirmLabel}</Text>
        </Pressable>
      </View>
    );
  }
