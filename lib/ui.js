...
//  - `left` toont fysiek rechts (onthuld bij naar-links-vegen, richting 'right');
//    `right` toont fysiek links (richting 'left') — gesture-handler-conventie.
...
export function SwipeRow({ left, right, children, friction = 2, threshold = 44 }) {
  ...
  const onWillOpen = (direction) => {
    // 'right' = rechter-acties open (naar links geveegd) → de `left`-actie (verwijderen).
    // 'left'  = linker-acties open  (naar rechts geveegd) → de `right`-actie (uitstellen).
    const action = direction === 'right' ? left : right;
    ref.current?.close?.();
    action?.onTrigger?.();
  };
