...
import {
  colors, radius, type, space, screenPadding, elevation, touchTarget, hitSlopFor,
} from './theme';
...
export function Button({
  ...
  disabled, loading, icon, fullWidth = true, accessibilityHint, style,
}) {
  const palette = {
    primary: { bg: colors.forest,     bgPressed: colors.forestSoft, fg: colors.onDark, border: 'transparent' },
    accent:  { bg: colors.ocher,      bgPressed: '#CE9531',         fg: colors.forest, border: 'transparent' },
...
// ===========================================================================
export function IconButton({ icon, onPress, accessibilityLabel, tint = colors.ink, size = 24, disabled, style }) {
  return (
    <Pressable
      onPress={onPress}
      disabled={disabled}
...
// ===========================================================================
export function FAB({ icon = 'add', label, onPress, accessibilityLabel, style }) {
  ...
  return (
    ...
    >
      <Icon name={icon} size={extended ? 20 : 26} color={colors.forest} weight="bold" />
      {extended ? <Text style={[type.button, { color: colors.forest }]}>{label}</Text> : null}
    </Pressable>
  );
}
...
// ===========================================================================
export function Stepper({ value, onChange, min = 1, max = 99, step = 1, formatValue, accessibilityLabel }) {
  ...
  const btn = (label, onPress, disabled, sign) => (
    <Pressable
      ...
      style={{ width: touchTarget, height: touchTarget, alignItems: 'center', justifyContent: 'center', opacity: disabled ? 0.35 : 1 }}
    >
      <Text style={{ fontSize: 24, color: colors.forest, marginTop: -2 }}>{sign}</Text>
    </Pressable>
  );
  return (
    <View
    ...
    >
      {btn('Minder', dec, value <= min, '−')}
      <Text style={[type.title, { minWidth: 36, textAlign: 'center' }]}>
        {formatValue ? formatValue(value) : value}
      </Text>
      {btn('Meer', inc, value >= max, '+')}
    </View>
  );
}
...
// bovenop `ReanimatedSwipeable` (react-native-gesture-handler): naar LINKS vegen
// voert de `left`-actie uit (standaardgebruik: verwijderen), naar RECHTS vegen de
// `right`-actie (standaardgebruik: uitstellen). De acties zijn declaratieve
...
// ===========================================================================
export function Chip({ label, icon, active, color, onPress }) {
  const tint = color || colors.forest;
  const fg = active ? colors.onDark : colors.inkSoft;
  return (
    <Pressable
      onPress={onPress}
      accessibilityRole="button"
...
// ===========================================================================
export function Checkbox({ checked, onPress, color = colors.forest, size = 26, shape = 'square', accessibilityLabel }) {
...
export function Badge({ label, tone = 'neutral' }) {
  ...
  return (
    <View style={{ backgroundColor: tones.bg, borderRadius: radius.pill, paddingHorizontal: space.sm, paddingVertical: 2, alignSelf: 'flex-start' }}>
...
// ===========================================================================
export function ModalHeader({ title, onClose, onConfirm, confirmLabel = 'Bewaar', cancelLabel = 'Annuleer', busy, backLabel }) {
  if (onConfirm) {
    return (
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: space.lg, paddingVertical: space.md }}>
        <Pressable onPress={onClose} hitSlop={10} accessibilityRole="button" accessibilityLabel={cancelLabel}>
          <Text style={[type.title, { fontWeight: '600', color: colors.inkSoft }]}>{cancelLabel}</Text>
        </Pressable>
        <Text style={type.title}>{title}</Text>
        <Pressable onPress={onConfirm} disabled={busy} hitSlop={10} accessibilityRole="button" accessibilityLabel={confirmLabel} accessibilityState={{ busy: !!busy }}>
          <Text style={[type.title, { fontWeight: '800', color: colors.forest, opacity: busy ? 0.5 : 1 }]}>{confirmLabel}</Text>
        </Pressable>
      </View>
    );
  }
    ...
    <View style={{ paddingHorizontal: space.lg, paddingVertical: space.md }}>
      {backLabel ? (
        ...
        >
          ...
          <Text style={[type.title, { color: colors.forest, fontWeight: '700' }]} numberOfLines={1}>{backLabel}</Text>
        </Pressable>
      ) : null}
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <Text style={[type.h1, { flex: 1 }]}>{title}</Text>
        {backLabel ? null : <IconButton icon="close" accessibilityLabel="Sluiten" tint={colors.inkSoft} onPress={onClose} />}
      </View>
    </View>
  );
}
...
export function Editor({
  ...
  children, scrollRef, contentContainerStyle, dirty = false,
}) {
...
export function BottomSheet({ visible, onClose, children, maxHeight = '90%', avoidKeyboard = false }) {
  ...
  // Omlaag-swipen om te sluiten (UX, batch 2) — net als een nette native drawer.
  // De Pan zit op het grijp-handvat bovenaan (niet op de hele sheet), zodat een
  // scrollende inhoud (bv. de filtersheet) gewoon blijft scrollen. Voorbij de drempel
  // of met genoeg snelheid → sluiten; de Modal's eigen slide-animatie schuift 'm uit.
  const ty = useSharedValue(0);
  useEffect(() => { if (visible) ty.value = 0; }, [visible]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const panel = (
    <View style={{ flex: 1, justifyContent: 'flex-end' }}>
      ...
      >
        {/* Grijp-handvat: de affordance + het sleepgebied voor swipe-to-dismiss. */}
        <GestureDetector gesture={pan}>
          <View accessibilityRole="adjustable" accessibilityLabel={t('common.close')}
            style={{ paddingTop: space.sm, paddingBottom: space.xs, alignItems: 'center' }}>
