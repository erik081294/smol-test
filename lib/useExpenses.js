...
export function useExpenses() {
  // Gegate op 'kosten' (ARCH-3) via de gedeelde primitive — geen losse gate-kopie.
  const activeId = useGatedHouseholdId('kosten');
  const { user } = useAuth();

  // Stale-while-revalidate + huishouden-wissel-reset via de gedeelde hook (we cachen de
  // genormaliseerde rijen — wat consumers lezen). De load/realtime/CRUD hieronder is
  // uitgaven-specifiek (venster + exactTotals + dubbele-tabel-realtime + RPC's).
  const key = activeId ? cacheKey('expenses', activeId) : null;
  const [expenses, setExpenses, loading, setLoading] = useCachedCollection(key);
  ...
  // Laadfout (offline/server). Blootgelegd zodat het Kosten-scherm een nette foutstaat toont
  // i.p.v. via computeBalances([]) onterecht "iedereen staat quitte" (P0-review 2026-07-02).
  const [error, setError] = useState(null);

  const load = useCallback(async () => {
    if (!activeId) { setExpenses([]); setExactTotals(null); setError(null); setLoading(false); return; }
    const { data, error: qErr } = await runResult(
      ...
      { context: 'uitgaven laden' }
    );
    if (qErr) {
      // Transiënte fout: láát de bestaande (gecachete) lijst staan i.p.v. 'm te blanken,
      // en markeer de fout zodat het scherm 'm kan tonen (banner + opnieuw proberen).
      setError(qErr);
      setLoading(false);
      return;
    }
  ...
  // Optimistisch verwijderen + rollback bij fout (spiegelt useCollection.remove). Nodig
  // omdat Supabase-realtime DELETE-events vaak niet uitzendt (replica identity): zonder de
  // lokale verwijdering blijft de rij in `expenses` staan — verborgen door de pending-undo,
  // maar zodra die afloopt komt 'ie terug tot de volgende volledige reload.
  const deleteExpense = (id) => {
    ...
    return mutate(
      ...
      { context: 'uitgave verwijderen' }
    ).catch((e) => { if (prev) setExpenses(prev); throw e; });
  };
  ...
  return { expenses: visibleExpenses, loading, error, reload: load, addExpense, updateExpense, deleteExpense, exactTotals, user };
}
