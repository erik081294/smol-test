...
function Bubble({ item, onAction, onEdit, onChoice }) {
  ...
  return (
    <View style={{
      ...
      {mine
        ...
        : item.tree.length > 0
          ? <AssistantMessageView tree={item.tree} onAction={onAction} onEdit={onEdit} onChoice={onChoice} />
          : item.text
    ...
    <View style={{ flex: 1 }}>
      {messages.length === 0 ? (
        ...
        <FlatList
          ...
          // exact dezelfde route als vrij typen of een chip-tik.
          renderItem={({ item }) => <Bubble item={item} onAction={resolveAction} onEdit={openEdit} onChoice={submit} />}
