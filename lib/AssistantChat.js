...
function Bubble({ item, onAction, onEdit, onChoice, onConfirmed }) {
  ...
  return (
    <View style={{
      ...
      {mine
        ...
        : item.tree.length > 0
          ? <AssistantMessageView tree={item.tree} onAction={onAction} onEdit={onEdit} onChoice={onChoice} onConfirmed={onConfirmed} />
          : item.text
...
export function AssistantChat({ assistant, autoFocus = false }) {
  const { enabled, messages, busy, stream, send, stop, retry, canRetry, choices, resolveAction, followUp, loadProposal, memberNames } = assistant;
    ...
    <View style={{ flex: 1 }}>
      {messages.length === 0 ? (
        ...
        <FlatList
          ...
          data={messages}
          keyExtractor={(m) => m.id}
          // De choice-kaart (AI-16) stuurt zijn reply als gewone gebruikersbeurt —
          // exact dezelfde route als vrij typen of een chip-tik. Na een geslaagd
          // akkoord (AI-18) volgt één vervolg-beurt: bevestiging + vervolgstap.
          renderItem={({ item }) => <Bubble item={item} onAction={resolveAction} onEdit={openEdit} onChoice={submit} onConfirmed={followUp} />}
