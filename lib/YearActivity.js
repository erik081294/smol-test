...
import { Card, SectionHeader, AvatarSelect, IconButton, Empty } from './ui';
...
const weekdayName = (weekday) =>
  format(new Date(2023, 0, 1 + weekday), 'EEEE', { locale: dateLocale() });
...
export function YearActivity({ members = [] }) {
  ...
  // Jaarwissel: de gemarkeerde dag hoort bij het oude jaar → wis 'm.
  const setYear = useCallback((y) => { setYearRaw(y); setSelected(null); }, []);

  const counts = useMemo(() => countsByDay(completions, { assigneeId }), [completions, assigneeId]);
    ...
    >
      ...
      {members.length > 1 ? (
        <AvatarSelect
          ...
          onSelect={(id) => { setAssigneeId(id); setSelected(null); }}
        />
      ) : null}
        ...
        {selected ? (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: space.sm, marginTop: space.md, paddingTop: space.md, borderTopWidth: 1, borderTopColor: colors.line }}>
            ...
            <Text style={[type.body, { flex: 1 }]}>
              <Text style={{ fontWeight: '700' }}>{format(selected.date, 'EEEE d MMMM', { locale: dateLocale() })}</Text>
              {'  ·  '}
