...
export function YearActivity({ members = [] }) {
  ...
  // Stabiele callback zodat de gememoiseerde DayCells niet allemaal herrenderen bij
  // een dag-selectie (setSelected is zelf stabiel, dus lege deps volstaat).
  const handleSelectDay = useCallback((cell) => setSelected({ key: cell.key, date: cell.date }), []);
        ...
        {selected ? (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: space.sm, marginTop: space.md, paddingTop: space.md, borderTopWidth: 1, borderTopColor: colors.line }}>
            ...
            <Text style={[type.body, { flex: 1 }]}>
              <Text style={{ fontFamily: font.semi }}>{cap(format(selected.date, 'EEEE d MMMM', { locale: dateLocale() }))}</Text>
              {'  ·  '}
