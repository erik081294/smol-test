...
// `existingTasks` (UXR-10 #8): de al bestaande, nog-openstaande schoonmaaktaken
// ([{ zone, title }]) — zodat een rooster bovenop bestaande taken geen dúbbele taak
// aanmaakt (vroeger kreeg elke room een taak, ook als die zone-taak al bestond → twee
// identieke "Badkamer schoonmaken" naast elkaar). Dedup-sleutel = zone + titel
// (genormaliseerd), zodat dezelfde titel in een ándere zone wél is toegestaan.
function buildSchedule(rooms, { existingZones = [], existingTasks = [], startDate = new Date() } = {}) {
  ...
  const taskKey = (zone, title) => `${norm(zone)}|${norm(title)}`;
  const existingTaskKeys = new Set((existingTasks ?? []).map((tk) => taskKey(tk.zone, tk.title)));
  const taskSeen = new Set();
  const tasks = [];
  for (const room of rooms) {
    const key = taskKey(room.zone, room.title);
    if (existingTaskKeys.has(key) || taskSeen.has(key)) continue;
    taskSeen.add(key);
    tasks.push({
      title: room.title,
      category: 'huishouden',
