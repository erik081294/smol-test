...
// De vier filter-assen uit het ontwerp. 'member' (TML-7) en 'subgroup' (TML-8)
// staan alvast in het DB-CHECK maar hebben hier nog geen beslislogica.
 ...
 * `item` draagt per as zijn waarde: `{ module, eventType }` — een handgeschreven
 * bericht heeft geen eventType en valt dus alleen onder assen die het wél draagt.
 ...
 * @param {{ module?: string, eventType?: string } | null} [item]
...
export function visibleOnTimeline(item, { householdDisabled = {}, userDisabled = {} } = {}) {
  ...
  const axes = [['module', item.module], ['event_type', item.eventType]];
