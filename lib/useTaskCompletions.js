...
export function useTaskCompletions() {
  ...
  // Laadfout blootgelegd zodat het eerlijkheidsoverzicht een banner + retry toont i.p.v. een
  // leeg "niemand deed iets" dat niet van een mislukte load te onderscheiden is (P0-review).
  const [error, setError] = useState(null);

  const load = useCallback(async () => {
    if (!activeId) { setCompletions([]); setExactCounts(null); setError(null); setLoading(false); return; }
  ...
  return { completions, loading, error, reload: load, exactCounts, user };
}
